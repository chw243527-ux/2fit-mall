...
function _totalProductStock(data) {
  const stockData = data?.stockData;
  if (stockData && typeof stockData === 'object') {
    return Object.values(stockData).reduce((sizeTotal, colorMap) => {
      if (!colorMap || typeof colorMap !== 'object') return sizeTotal;
      return sizeTotal + Object.values(colorMap).reduce((sum, value) => {
        const qty = Number(value);
        return sum + (Number.isFinite(qty) ? Math.max(0, Math.floor(qty)) : 0);
      }, 0);
    }, 0);
  }
  const fallback = Number(data?.stockCount || 0);
...
exports.onProductRestocked = onDocumentUpdated(
  { document: 'products/{productId}', secrets: [RESEND_API_KEY] },
  async (event) => {
    const before = event.data?.before?.data();
    const after = event.data?.after?.data();
    if (!before || !after) return;
    const beforeTotal = _totalProductStock(before);
    const afterTotal = _totalProductStock(after);
    if (beforeTotal > 0 || afterTotal <= 0) return;

    const productId = String(event.params.productId || '').trim();
    const productName = String(after.name || before.name || '상품').slice(0, 120);
    if (!productId) return;
    try {
      const alertSnapshot = await db.collection('restock_alerts')
        .where('productId', '==', productId).get();
      for (const alertDoc of alertSnapshot.docs) {
        const alert = alertDoc.data() || {};
        if (alert.notified === true || alert.isNotified === true) continue;
        const userId = String(alert.userId || '').trim();
        if (!userId) continue;
        const userSnap = await db.collection('users').doc(userId).get();
        const user = userSnap.data() || {};
        if (user.orderNotificationsEnabled === false) continue;

        const notificationRef = db.collection('notifications').doc();
        await notificationRef.set({
          id: notificationRef.id, userId, title: '🔔 재입고 알림',
          body: `"${productName}" 상품이 재입고되었습니다!`, type: 'restock',
          productId, isRead: false, createdAt: FieldValue.serverTimestamp(),
        });
        ...
        if (token) {
          try {
            await getMessaging().send({
              token,
              notification: {
                title: '2FIT MALL 재입고 알림',
                body: `"${productName}" 상품이 재입고되었습니다.`,
              },
              data: { type: 'restock', productId, notificationId: notificationRef.id },
            });
          } catch (pushError) {
            console.error('restock FCM delivery failed', {
              code: _errorCode(pushError, 'restock-push-failed'),
            });
          }
        }
        ...
        if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
          try {
            await _sendResendEmail({
              to: email,
              subject: `[2FIT MALL] ${productName} 재입고 알림`,
              text: `신청하신 "${productName}" 상품이 재입고되었습니다. 2FIT MALL에서 확인해 주세요.`,
              html: `<p>신청하신 <strong>${productName}</strong> 상품이 재입고되었습니다.</p><p>2FIT MALL에서 확인해 주세요.</p>`,
            });
          } catch (emailError) {
            console.error('restock email delivery failed', {
              code: _errorCode(emailError, 'restock-email-failed'),
            });
          }
        }
        await alertDoc.ref.update({
          notified: true, isNotified: true, notifiedAt: FieldValue.serverTimestamp(),
        });
      }
    } catch (error) {
      console.error('onProductRestocked failed', {
