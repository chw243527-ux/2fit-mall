...
def wloc(text: str) -> str:
    """loc.t() 형식 (context 없이 loc 사용하는 곳)"""
    key = make_key(text)
    safe = text.replace("'", "\\'")
    return f"loc.t('{key}', '{safe}')"

def process_file(path: str, replacements: list) -> int:
    p = Path(path)
    if not p.exists():
    ...
    for old, new in replacements:
        ...
        if old in src:
            ...
            print(f"  [!] NOT FOUND in {path}: {old[:80]!r}")
    if src != orig:
...
# ─────────────────────────────────────────────
# 1. group_order_test_screen.dart
# ─────────────────────────────────────────────
test_replacements = [
    # _sectionTitle() 호출들
    ...
    # Text with $ interpolation — 스킵 불가 수동
    # '생성 대상: ${user.name} (${user.id})' → $-interpolation 포함, Text로 wrapping
    ("? Text('생성 대상: ${user.name} (${user.id})',",
     f"? Text('생성 대상: ${{user.name}} (${{user.id}})',"),  # keep as-is (already has $)

    # '• 생성된 주문은 ...' 안내 텍스트
    ("'• 생성된 주문은 로그인 유저 ID로 저장 → 마이페이지에서 즉시 확인 가능\\n• 기성품/단체주문 둘 다 생성 가능 · 상태(배송중/완료 등) 선택 가능\\n• 주문번호 TEST_GRP_ / TEST_PERS_ 로 시작 → 상단 🗑 버튼으로 일괄 삭제',",
     f"{w('생성된_주문은_로그인_유저_ID_저장_마이페이지_확인_단체_기성품_TEST_GRP')}," .replace(
         "생성된_주문은_로그인_유저_ID_저장_마이페이지_확인_단체_기성품_TEST_GRP",
         make_key('• 생성된 주문은 로그인 유저 ID로 저장 → 마이페이지에서 즉시 확인 가능')
     )
    ),

    # chip 에서 '기성품'/'단체' — DB값으로 사용 안되므로 번역 적용
    ("_chip(p.orderType == 'personal' ? '기성품' : '단체', p.color)",
     f"_chip(p.orderType == 'personal' ? {w('기성품')} : {w('단체')}, p.color)"),

    # '기성품 (personal)' / '단체주문 (group)' 토글
...
# Note: '• 생성된 주문은...' 는 긴 문자열이라 직접 처리
test_replacements2 = [
    ("'• 생성된 주문은 로그인 유저 ID로 저장 → 마이페이지에서 즉시 확인 가능\\n• 기성품/단체주문 둘 다 생성 가능 · 상태(배송중/완료 등) 선택 가능\\n• 주문번호 TEST_GRP_ / TEST_PERS_ 로 시작 → 상단 🗑 버튼으로 일괄 삭제',",
     f"context.loc.t('생성된_주문_로그인_유저_ID_마이페이지_확인_가능', '• 생성된 주문은 로그인 유저 ID로 저장 → 마이페이지에서 즉시 확인 가능\\n• 기성품/단체주문 둘 다 생성 가능 · 상태(배송중/완료 등) 선택 가능\\n• 주문번호 TEST_GRP_ / TEST_PERS_ 로 시작 → 상단 🗑 버튼으로 일괄 삭제'),"),
]

process_file('lib/screens/orders/group_order_test_screen.dart',
    [
        ("_sectionTitle('빠른 프리셋', Icons.flash_on_rounded)",
         f"_sectionTitle({w('빠른 프리셋')}, Icons.flash_on_rounded)"),
        ("_sectionTitle('커스텀 주문 생성', Icons.tune_rounded)",
        ...
        ("_toast('테스트 주문 ${total}건 삭제 완료');",
         "_toast('테스트 주문 ${total}건 삭제 완료');"),  # $-interpolation 스킵
        ("_toast('삭제 오류: $e', error: true);",
         "_toast('삭제 오류: $e', error: true);"),  # $-interpolation 스킵
    ]
)
...
# ─────────────────────────────────────────────
process_file('lib/screens/policy/privacy_policy_screen.dart',
    [
        # _buildSection 제목들
        ("'제1조 (수집하는 개인정보 항목)',",
        ...
        ("'2FIT MALL은 회원가입 및 서비스 이용을 위해 아래와 같은 개인정보를 수집합니다.\\n\\n'\n                '• 필수항목: 이름, 이메일 주소, 비밀번호, 휴대폰 번호\\n'\n                '• 선택항목: 마케팅 수신 동의\\n'\n                '• 자동수집: 서비스 이용기록, 접속 로그, 쿠키, IP 주소',",
         f"context.loc.t('개인정보_수집_항목_내용', '2FIT MALL은 회원가입 및 서비스 이용을 위해 아래와 같은 개인정보를 수집합니다.\\n\\n• 필수항목: 이름, 이메일 주소, 비밀번호, 휴대폰 번호\\n• 선택항목: 마케팅 수신 동의\\n• 자동수집: 서비스 이용기록, 접속 로그, 쿠키, IP 주소'),"),
        ("'제2조 (개인정보의 수집 및 이용목적)',",
         f"{w('제2조 (개인정보의 수집 및 이용목적)')},"),
        ("'• 회원가입 및 본인 확인\\n'\n                '• 서비스 제공 및 계약 이행\\n'\n                '• 주문/배송/결제 처리\\n'\n                '• 주문 확인·배송 안내 카카오 알림톡 발송\\n'\n                '• 고객 문의 및 불만 처리\\n'\n                '• 마케팅 및 광고 활용 (동의 시)',",
         f"context.loc.t('개인정보_이용목적_내용', '• 회원가입 및 본인 확인\\n• 서비스 제공 및 계약 이행\\n• 주문/배송/결제 처리\\n• 주문 확인·배송 안내 카카오 알림톡 발송\\n• 고객 문의 및 불만 처리\\n• 마케팅 및 광고 활용 (동의 시)'),"),
        ("'제3조 (개인정보 보유 및 이용기간)',",
         f"{w('제3조 (개인정보 보유 및 이용기간)')},"),
        ("'회원 탈퇴 시 즉시 삭제합니다. 단, 관련 법령에 따라 아래 기간 동안 보관합니다.\\n\\n'\n                '• 계약/청약철회 기록: 5년 (전자상거래법)\\n'\n                '• 소비자 불만/분쟁처리 기록: 3년\\n'\n                '• 접속 로그: 3개월 (통신비밀보호법)',",
         f"context.loc.t('개인정보_보유기간_내용', '회원 탈퇴 시 즉시 삭제합니다. 단, 관련 법령에 따라 아래 기간 동안 보관합니다.\\n\\n• 계약/청약철회 기록: 5년 (전자상거래법)\\n• 소비자 불만/분쟁처리 기록: 3년\\n• 접속 로그: 3개월 (통신비밀보호법)'),"),
        ("'제4조 (개인정보 제3자 제공)',",
         f"{w('제4조 (개인정보 제3자 제공)')},"),
...
# ─────────────────────────────────────────────
process_file('lib/screens/policy/terms_of_service_screen.dart',
    [
        # _InfoRow 레이블
        ...
        ("'본 약관은 주식회사 2FIT Korea(이하 \"회사\")가 운영하는 2FIT MALL 쇼핑몰 서비스의 이용조건 및 절차, 회사와 이용자 간의 권리·의무 관계를 규정함을 목적으로 합니다.',",
         f"context.loc.t('이용약관_제1조_목적_내용', '본 약관은 주식회사 2FIT Korea(이하 \"회사\")가 운영하는 2FIT MALL 쇼핑몰 서비스의 이용조건 및 절차, 회사와 이용자 간의 권리·의무 관계를 규정함을 목적으로 합니다.'),"),
        ("'제2조 (회원가입)',",
         f"{w('제2조 (회원가입)')},"),
        ("'• 만 14세 이상 이용 가능합니다.\\n'\n                '• 타인의 정보 도용 가입은 금지됩니다.\\n'\n                '• 허위 정보 제공 시 서비스 이용이 제한될 수 있습니다.',",
         f"context.loc.t('이용약관_제2조_내용', '• 만 14세 이상 이용 가능합니다.\\n• 타인의 정보 도용 가입은 금지됩니다.\\n• 허위 정보 제공 시 서비스 이용이 제한될 수 있습니다.'),"),
        ("'제3조 (서비스 이용)',",
         f"{w('제3조 (서비스 이용)')},"),
        ("'• 서비스는 연중무휴 24시간 제공을 원칙으로 합니다.\\n'\n                '• 시스템 정기점검, 천재지변 등 불가피한 경우 서비스가 중단될 수 있습니다.\\n'\n                '• 주문 완료 후 카카오 알림톡으로 주문확인·배송 안내를 발송합니다.',",
         f"context.loc.t('이용약관_제3조_내용', '• 서비스는 연중무휴 24시간 제공을 원칙으로 합니다.\\n• 시스템 정기점검, 천재지변 등 불가피한 경우 서비스가 중단될 수 있습니다.\\n• 주문 완료 후 카카오 알림톡으로 주문확인·배송 안내를 발송합니다.'),"),
        ("'제4조 (구매 및 결제)',",
         f"{w('제4조 (구매 및 결제)')},"),
        ("'• 결제 수단: 신용카드, 가상계좌, 계좌이체, 간편결제 (토스페이먼츠)\\n'\n                '• 주문 완료 후 결제 확인 시 배송이 시작됩니다.\\n'\n                '• 단순 변심에 의한 반품은 수령 후 7일 이내 가능합니다.\\n'\n                '• 상품 하자의 경우 수령 후 3개월 이내 교환/환불이 가능합니다.',",
         f"context.loc.t('이용약관_제4조_내용', '• 결제 수단: 신용카드, 가상계좌, 계좌이체, 간편결제 (토스페이먼츠)\\n• 주문 완료 후 결제 확인 시 배송이 시작됩니다.\\n• 단순 변심에 의한 반품은 수령 후 7일 이내 가능합니다.\\n• 상품 하자의 경우 수령 후 3개월 이내 교환/환불이 가능합니다.'),"),
        ("'제5조 (교환 및 환불)',",
         f"{w('제5조 (교환 및 환불)')},"),
        ("'• 교환/환불은 수령 후 7일 이내 신청 가능합니다.\\n'\n                '• 단체복/맞춤제작 상품은 교환·환불이 불가합니다.\\n'\n                '• 상품 불량/오배송의 경우 100% 교환 또는 환불 처리합니다.\\n'\n                '• 환불은 결제 취소 후 3~5 영업일 내 처리됩니다.\\n'\n                '• 반품 배송비는 구매자 부담이며, 상품 불량 시 회사가 부담합니다.',",
         f"context.loc.t('이용약관_제5조_내용', '• 교환/환불은 수령 후 7일 이내 신청 가능합니다.\\n• 단체복/맞춤제작 상품은 교환·환불이 불가합니다.\\n• 상품 불량/오배송의 경우 100% 교환 또는 환불 처리합니다.\\n• 환불은 결제 취소 후 3~5 영업일 내 처리됩니다.\\n• 반품 배송비는 구매자 부담이며, 상품 불량 시 회사가 부담합니다.'),"),
        ("'제6조 (배송)',",
         f"{w('제6조 (배송)')},"),
        ("'• 주문 확인 후 1~3 영업일 이내 발송합니다.\\n'\n                '• 단체복 맞춤제작은 제작 기간(7~14일)이 소요됩니다.\\n'\n                '• 배송비는 상품 페이지에 표시된 금액에 따릅니다.\\n'\n                '• 배송 시작 시 카카오 알림톡으로 운송장 번호를 안내합니다.',",
         f"context.loc.t('이용약관_제6조_내용', '• 주문 확인 후 1~3 영업일 이내 발송합니다.\\n• 단체복 맞춤제작은 제작 기간(7~14일)이 소요됩니다.\\n• 배송비는 상품 페이지에 표시된 금액에 따릅니다.\\n• 배송 시작 시 카카오 알림톡으로 운송장 번호를 안내합니다.'),"),
        ("'제7조 (금지행위)',",
         f"{w('제7조 (금지행위)')},"),
        ("'• 타인의 계정 무단 사용\\n'\n                '• 서비스 운영 방해\\n'\n                '• 허위 리뷰 작성\\n'\n                '• 불법 콘텐츠 유포',",
         f"context.loc.t('이용약관_제7조_내용', '• 타인의 계정 무단 사용\\n• 서비스 운영 방해\\n• 허위 리뷰 작성\\n• 불법 콘텐츠 유포'),"),
        ("'제8조 (면책조항)',",
         f"{w('제8조 (면책조항)')},"),
        ("'천재지변, 전쟁 등 불가항력으로 인한 서비스 중단에 대해 회사는 책임을 지지 않습니다.',",
         f"{w('천재지변_전쟁_등_불가항력으로_인한_서비스_중단에_대_b0a15c')},"),
        ("'제9조 (준거법 및 관할법원)',",
         f"{w('제9조 (준거법 및 관할법원)')},"),
        ("'본 약관은 대한민국 법률에 따라 규율되며, 서비스 이용과 관련한 분쟁은 회사 소재지 관할 법원(전주지방법원 남원지원)을 전속 관할로 합니다.',",
         f"context.loc.t('이용약관_제9조_내용', '본 약관은 대한민국 법률에 따라 규율되며, 서비스 이용과 관련한 분쟁은 회사 소재지 관할 법원(전주지방법원 남원지원)을 전속 관할로 합니다.'),"),
    ]
)
...
# ─────────────────────────────────────────────
process_file('lib/screens/orders/checkout_screen.dart',
    [
        # infoRow() 레이블
        ...
        # 팀원 명단 — $-interpolation 포함이므로 Text 그대로 두고 key만 번역
        # printTypeLabels
        ...
        ("'• 입금자명이 주문자명과 다를 경우 입금 확인이 지연될 수 있습니다.\\n• 주문 완료 후 표시되는 계좌로 24시간 이내 입금해 주세요.',",
         f"context.loc.t('입금자명_주문자명_다를_경우_지연_24시간', '• 입금자명이 주문자명과 다를 경우 입금 확인이 지연될 수 있습니다.\\n• 주문 완료 후 표시되는 계좌로 24시간 이내 입금해 주세요.'),"),
        # 무통장입금 다이얼로그
        ("'무통장입금 안내',",
         f"{w('무통장입금 안내')},"),
        ("'입금 계좌',",
...
# ─────────────────────────────────────────────
# 5. order_guide_screen.dart
# ─────────────────────────────────────────────
process_file('lib/screens/orders/order_guide_screen.dart',
    [
        # 안내 확인 체크박스 텍스트
        ("'주문 안내 내용을 모두 확인했습니다.',",
        ...
        ("'단체 주문서',",
         f"{w('단체 주문서')},"),
        # _groupOrderFields — 'field' 값은 번역키로 이전 세션에서 동적 번역 적용됨
        # 따라서 example 값도 번역 가능하면 적용, field는 이미 동적 처리 됨
        # 취소/교환/환불
        ...
        ("'※ 도서/산간 지역은 추가 배송비가 발생할 수 있습니다.\\n※ 배송 관련 문의는 고객센터로 연락해주세요.',",
         f"context.loc.t('도서_산간_지역_추가_배송비_배송_관련_문의_고객', '※ 도서/산간 지역은 추가 배송비가 발생할 수 있습니다.\\n※ 배송 관련 문의는 고객센터로 연락해주세요.'),"),
        # FAQ
        ...
        ("'⚠️ 추가구매는 마이페이지 > 기존 주문내역에서 신청하실 수 있습니다.',",
         f"context.loc.t('추가구매_마이페이지_기존_주문내역_신청', '⚠️ 추가구매는 마이페이지 > 기존 주문내역에서 신청하실 수 있습니다.'),"),
    ]
)


# ─────────────────────────────────────────────
# 6. group_order_only_screen.dart
# ─────────────────────────────────────────────
process_file('lib/screens/orders/group_order_only_screen.dart',
    [
        # sub 레이블
        ("sub: '단체전용',",
         f"sub: {w('단체전용')},"),
...
# 7. app_drawer.dart
# ─────────────────────────────────────────────
process_file('lib/widgets/app_drawer.dart',
    [
        # '단체주문하기' 문자열 2곳 (loc.t 형식으로)
        # L73: final allTabName = loc.t('전체 $mainCat', '전체 $mainCat'); — $ 포함, 스킵
        # L144: label: loc.t('단체주문하기', '단체주문하기'), — 이미 처리됨
        # L616: '© 2024 2FIT KOREA' — 저작권 표기, 번역 불필요 스킵
    ]
)


# ─────────────────────────────────────────────
# 8. size_profile_screen.dart
# ─────────────────────────────────────────────
process_file('lib/screens/mypage/size_profile_screen.dart',
    [
        # L66: SnackBar 메시지
        ("SnackBar(content: Text('최대 ${SizeProfileService.maxProfiles}개까지 저장 가능합니다.')),",
         "SnackBar(content: Text('최대 ${SizeProfileService.maxProfiles}개까지 저장 가능합니다.')),"),  # $-interpolation 스킵
        # L237: _sizeType = '성인' — DB/logic 값, 스킵
        # L390: headers 목록
...
# ─────────────────────────────────────────────
# 9. group_order_landing_screen.dart
# ─────────────────────────────────────────────
process_file('lib/screens/orders/group_order_landing_screen.dart',
    [
        # 잔여 5개 확인 후 처리
        ("'단체주문하기',",
         f"{w('단체주문하기')},"),
