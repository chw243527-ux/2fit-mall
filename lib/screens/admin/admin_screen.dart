  ...
  final List<Map<String, dynamic>> _chatSessions = [
    {
      ...
      'messages': <ChatMessage>[
        ChatMessage(
            text: '주문 상태 확인',
            originalText: '주문 상태 확인',
            isUser: true,
            time: DateTime.now().subtract(const Duration(minutes: 5))),
        ChatMessage(
            text: '주문 번호를 알려주시면 주문 상태를 확인해 드리겠습니다.',
            isUser: false,
...
// ignore: unused_element
    ...
    try {
      ...
      for (final doc in snap.docs) {
        ...
        if (rawAt is String) {
          ...
          createdAt =
              fallback is Timestamp ? fallback.toDate() : DateTime.now();
        }
        ...
        if (personList.isNotEmpty) {
          descParts.add('사이즈 변경 ${personList.length}명');
        }
        ...
        requests.add({
          ...
          'designConfirmedImageUrl': (d['customOptions']
                      as Map<String, dynamic>?)?['designConfirmedImageUrl']
                  as String? ??
              '',
        });
      }
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      final hasNotif =
                          i == 4 && _pendingChatCount > 0; // 채팅상담 = index 4
                        ...
                        children: [
                          Expanded(
                            child: StreamBuilder<List<AdminNotification>>(
                              ...
                              builder: (ctx, _) {
                                final unread =
                                    AdminNotificationStore.unreadCount;
                                return TextButton.icon(
                                  ...
                                  label: Text(unread > 0 ? '알림 $unread' : '알림',
                                      style: TextStyle(
                                          fontSize: 11,
                                          color: unread > 0
                                              ? AppColors.accent
                  ...
                  IconButton(
                    icon: Icon(
                        unread > 0
                            ...
                            : Icons.notifications_outlined,
                        size: 19,
                        color: unread > 0 ? AppColors.accent : Colors.white),
                    onPressed: () {
                      setState(() {});
                      _showAdminNotifications();
                    },
    ...
    return StreamBuilder<List<OrderModel>>(
      ...
      builder: (context, snapshot) {
        ...
        final inProgress = allOrders
            ...
            .length;
        final deliveredToday = allOrders
            .where((o) =>
        ...
        final personalOrders =
            allOrders.where((o) => o.orderType == 'personal').length;
        final groupOrders = allOrders
            .where((o) => o.orderType == 'group' || o.orderType == 'additional')
            .length;
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _orderTypeRow('개인 주문', personalOrders, AppColors.info),
                      const SizedBox(height: 10),
                      _orderTypeRow('단체/추가제작', groupOrders, AppColors.primary),
                      const SizedBox(height: 10),
                      _orderTypeRow(
                          '총 주문', allOrders.length, const Color(0xFF00838F)),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _quickActionRow(Icons.add_box_rounded, '상품 추가',
                          AppColors.info, () => _tabCtrl.animateTo(6)),
                      const SizedBox(height: 6),
                      _quickActionRow(Icons.image_rounded, '배너 추가',
                          AppColors.success, () => _tabCtrl.animateTo(9)),
                      const SizedBox(height: 6),
                      _quickActionRow(
                          Icons.chat_rounded, '고객 채팅', AppColors.primary, () {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                      ...
                      const SizedBox(height: 6),
                      _quickActionRow(
                          Icons.download_rounded, '주문 내보내기', AppColors.error,
                          () {
  ...
  Widget _buildSalesStats(List<OrderModel> allOrders) {
    ...
    double calcRevenue(DateTime from, DateTime to) => allOrders
        .where((o) => !o.createdAt.isBefore(from) && !o.createdAt.isAfter(to))
        .fold(0.0, (s, o) => s + o.totalAmount);

    int calcCount(DateTime from, DateTime to) => allOrders
        .where((o) => !o.createdAt.isBefore(from) && !o.createdAt.isAfter(to))
        .length;
  ...
  Widget _salesRowV2(
      String period, String amount, int count, bool isCurrentPeriod) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      child: Row(
        children: [
          Flexible(
            ...
            child: Text(period,
                style: TextStyle(
                  fontSize: 13,
                  fontWeight:
                      isCurrentPeriod ? FontWeight.w700 : FontWeight.w400,
          ...
          Flexible(
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                fontWeight: FontWeight.w800,
                color:
                    isCurrentPeriod ? AppColors.info : AppColors.textSecondary,
              ),
              overflow: TextOverflow.ellipsis,
              textAlign: TextAlign.right,
            ),
    ...
    return StreamBuilder<List<OrderModel>>(
      ...
      builder: (context, snapshot) {
        ...
        var filtered = _orderFilterIdx == 0
            ? allOrders
            : allOrders
                .where((o) => o.status.label == _orderFilters[_orderFilterIdx])
                .toList();
                        ...
                        children: [
                          ...
                          ..._orderFilters.skip(1).map((s) => ListTile(
                                ...
                                trailing: _orderFilters[_orderFilterIdx] == s
                                    ...
                                    : null,
                                onTap: () {
                                  setState(() => _orderFilterIdx =
                                      _orderFilters.indexOf(s));
                                  Navigator.pop(context);
                                },
                              )),
                          ListTile(
              ...
              child: Row(
                children: [
                          ...
                          child: Icon(
                            ...
                            key: ValueKey(allSelected
                                ...
                                : _selectedOrderIds.isEmpty
                                    ? 'none'
                                    : 'partial'),
                            color: allSelected || _selectedOrderIds.isNotEmpty
                  ...
                  if (anySelected) ...[
                    // 선택 PDF 다운로드
                    GestureDetector(
                      onTap: () => _exportSelectedOrdersPdf(
                        filtered
                      ...
                      onTap: () async {
                        final ok = await showDialog<bool>(
                          ...
                          builder: (_) => AlertDialog(
                            ...
                            content: Text(
                                '선택한 ${_selectedOrderIds.length}건의 주문을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.'),
                            actions: [
                              TextButton(
                                  onPressed: () =>
                                      Navigator.pop(context, false),
  ...
  Future<void> _exportSelectedOrdersPdf(List<OrderModel> selectedOrders) async {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('PDF 파일 생성 중...', style: TextStyle(fontSize: 14)),
              ],
            ),
          ),
        ),
    ...
    try {
      ...
      final fileName = '2FIT_선택주문_${selectedOrders.length}건_$dateStr.pdf';
      await _handleFileDownload(
          bytes, fileName, selectedOrders.length, now, now);
    } catch (e) {
      if (!mounted) return;
      Navigator.of(context, rootNavigator: true).pop();
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _exportOrdersCSV(List<OrderModel> orders) {
    if (orders.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('내보낼 주문이 없습니다.'), backgroundColor: AppColors.primary),
      );
      return;
    }
    // 날짜별 내보내기 선택 다이얼로그
  ...
  Future<void> exportDesignRequestsToExcel(
    ...
    if (requests.isEmpty) {
      ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
          content: Text('내보낼 디자인 수정 요청이 없습니다.'),
          backgroundColor: AppColors.primary));
      return;
    }
    const mimeType =
    ...
    try {
      final bytes =
          await OrderExcelService.generateDesignRevisionExcelAsync(requests);
      if (kIsWeb) {
        ...
        if (!ctx.mounted) return;
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
            content: Text('웹 환경에서만 다운로드가 지원됩니다.'),
            backgroundColor: AppColors.textSecondary));
      }
    } catch (e) {
      if (!ctx.mounted) return;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setD) {
        ...
        if (exportType == '일일') {
          ...
          range = OrderExcelService.getWeekRange(
              DateTime.now().add(Duration(days: weekOffset * 7)));
        } else if (exportType == '월별') {
          range = OrderExcelService.getMonthRange(DateTime(
              DateTime.now().year, DateTime.now().month + monthOffset));
        ...
        String monthLabel() {
          final m =
              DateTime(DateTime.now().year, DateTime.now().month + monthOffset);
        ...
        Widget optBtn(String type, String label, String sub, Color color) {
          ...
          return Expanded(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: sel ? color : AppColors.surfaceGray,
                borderRadius: BorderRadius.circular(8),
        ...
        return AlertDialog(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      const SizedBox(width: 6),
                      optBtn('일별', '일별', '특정 하루', AppColors.info),
                      const SizedBox(width: 6),
                      optBtn('주별', '주별', '월~일 1주', AppColors.primary),
                    ]),
                    const SizedBox(height: 6),
                    Row(children: [
                      optBtn('월별', '월별', '1개월', const Color(0xFF00695C)),
                      const SizedBox(width: 6),
                      optBtn('기간선택', '기간선택', '직접 지정', AppColors.accent),
                      const SizedBox(width: 6),
                      optBtn('전체', '전체', '전체 주문', AppColors.success),
                    ]),
                    const SizedBox(height: 14),
          ...
          actions: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text('단체주문별 PDF 다운로드',
                  style: TextStyle(fontWeight: FontWeight.w700)),
              onPressed: () async {
                Navigator.pop(ctx);
  ...
  Future<void> _exportGroupPdfByRange({
    ...
    try {
      ...
      if (exportType == '일일') {
        ...
        finalRange = OrderExcelService.getWeekRange(
            DateTime.now().add(Duration(days: weekOffset * 7)));
      } else if (exportType == '월별') {
        finalRange = OrderExcelService.getMonthRange(
            DateTime(DateTime.now().year, DateTime.now().month + monthOffset));
      } else if (exportType == '기간선택') {
        finalRange = OrderExcelService.getCustomRange(rangeStart, rangeEnd);
      } else {
        finalRange =
            OrderExcelService.getCustomRange(DateTime(2020), DateTime.now());
      }
      ...
      final groupOnlyOrders = orders.where((o) {
        ...
        final hasTeamName =
            (o.customOptions?['teamName'] as String?)?.isNotEmpty == true;
        final hasPersons =
            (o.customOptions?['persons'] as List?)?.isNotEmpty == true;
  ...
  Future<void> _handleFileDownload(Uint8List bytes, String fileName,
      int orderCount, DateTime start, DateTime end) async {
    final isPdf = fileName.toLowerCase().endsWith('.pdf');
    final mimeType = isPdf
        ? 'application/pdf'
        : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    final formatLabel = isPdf ? 'PDF' : '엑셀';
          ...
          content: Row(children: [
            ...
            Expanded(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('$orderCount건 $formatLabel 다운로드 완료!',
                      style: const TextStyle(
                          fontWeight: FontWeight.w700, fontSize: 13)),
  ...
  void _showAutoUpdateOrdersDialog(List<OrderModel> orders) {
    ...
    final totalCount =
        toProcessing.length + toShipped.length + toDelivered.length;
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Row(
          children: [
            Icon(Icons.autorenew_rounded, color: AppColors.info),
            SizedBox(width: 8),
            Text('주문 상태 자동 업데이트',
  ...
  Future<void> _executeAutoUpdate(
    ...
    try {
      ...
      for (final o in [...toProcessing, ...toShipped, ...toDelivered]) {
        final newStatus = toProcessing.contains(o)
            ? OrderStatus.processing
            : toShipped.contains(o)
                ? OrderStatus.shipped
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                '✅ $updated건 자동 업데이트 완료${failed > 0 ? ' ($failed건 실패)' : ''}'),
            backgroundColor: AppColors.info,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setD) {
          ...
          if (exportType == '일일') {
            ...
            final base =
                DateTime.now().add(Duration(days: selectedWeekOffset * 7));
            range = OrderExcelService.getWeekRange(base);
          } else if (exportType == '월별') {
            final base = DateTime(DateTime.now().year,
                DateTime.now().month + selectedMonthOffset);
          ...
          if (exportType == '전체') {
            ...
            final fS = range.start;
            final fE = range.end;
            previewCount = allOrders
                .where((o) =>
                    !o.createdAt.isBefore(fS) && !o.createdAt.isAfter(fE))
                .length;
          } else {
            previewCount = 0;
          }
          ...
          // 월별 이름
          String monthLabel() {
            final m = DateTime(DateTime.now().year,
                DateTime.now().month + selectedMonthOffset);
            return '${m.year}년 ${m.month}월';
          }
          ...
          // 옵션 버튼 빌더
          Widget optBtn(
              String type, String label, String sub, Color activeColor) {
            final sel = exportType == type;
            return Expanded(
              child: GestureDetector(
                onTap: () => setD(() => exportType = type),
                child: Container(
                  padding: const EdgeInsets.symmetric(vertical: 9),
                  decoration: BoxDecoration(
                    color: sel ? activeColor : AppColors.surfaceGray,
                    borderRadius: BorderRadius.circular(8),
                    border: sel ? null : Border.all(color: AppColors.border),
                  ),
                  child: Column(
                            ...
                            child: Center(
                                child: Text(weekLabel(),
                                    style: const TextStyle(
                                        ...
                                        color: AppColors.primary)))),
                        IconButton(
                          onPressed: selectedWeekOffset < 0
                              ? () => setD(() => selectedWeekOffset++)
                              : null,
                          icon: Icon(Icons.chevron_right_rounded,
                              color: selectedWeekOffset < 0
                                  ? AppColors.primary
                                  : Colors.grey),
                          padding: EdgeInsets.zero,
                          constraints:
                        ...
                        children: [
                          ...
                          Row(children: [
                            ...
                            Expanded(
                                child: Text(
                              previewCount > 0
                                  ? '$previewCount건 확인됨 · 한글 지원 PDF 파일'
                  ...
                  try {
                    ...
                    if (exportType == '일일') {
                      ...
                      final base = DateTime.now()
                          .add(Duration(days: selectedWeekOffset * 7));
                      ...
                      finalRange = OrderExcelService.getCustomRange(
                          rangeStart, rangeEnd);
                    } else {
                      finalRange = OrderExcelService.getCustomRange(
                          DateTime(2020), DateTime.now());
                    }

                    final fStart = finalRange.start;
                    final fEnd = finalRange.end;
                    final finalOrders =
                        await OrderExcelService.getOrdersByDateRange(
                            fStart, fEnd);
                    ...
                    final endStr =
                        '${fEnd.year}${fEnd.month.toString().padLeft(2, "0")}${fEnd.day.toString().padLeft(2, "0")}';
                    final typeTag = {
                          '일일': 'daily',
                          '일별': 'day',
                          '주별': 'week',
                        ...
                        'export';
                    final fileName =
                        '2FIT_주문_${typeTag}_${startStr}_$endStr.pdf';
                    await _handleFileDownload(
                        bytes, fileName, finalOrders.length, fStart, fEnd);
                  } catch (e) {
                    if (!mounted) return;
                    Navigator.of(context, rootNavigator: true).pop();
                    ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _doExportCSV(List<OrderModel> orders, String fileName) {
    ...
    for (final o in orders) {
      final items = o.items
          .map((i) => '${i.productName}(${i.size}/${i.color})')
          .join(' | ');
      final qty = o.items.fold<int>(0, (total, i) => total + i.quantity);
      final date =
    ...
    if (kIsWeb) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('CSV 내보내기는 웹 환경에서만 지원됩니다.'),
            backgroundColor: AppColors.primary),
      );
    }
  }
  ...
  Widget _orderCard(OrderModel order) {
    ...
    bool isGroup =
        order.orderType == 'group' || order.orderType == 'additional';
    if (!isGroup) {
      final isGrpId =
          order.id.startsWith('GRP_') || order.id.startsWith('GROUP-');
      final hasTeamName =
          (order.customOptions?['teamName'] as String?)?.isNotEmpty == true;
      final hasPersons =
          (order.customOptions?['persons'] as List?)?.isNotEmpty == true;
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: const Border(
                    bottom: BorderSide(color: AppColors.surfaceGray)),
              ),
              child: Row(
                children: [
                  ...
                  _miniTag(
                      ...
                      order.orderType == 'additional'
                          ...
                          : (isGroup ? AppColors.primary : AppColors.info)),
                  const SizedBox(width: 4),
                  Container(
                    padding:
                            ...
                            Flexible(
                              child: Text(
                                order.userName.isNotEmpty
                                    ? order.userName
                                    : (opts?['managerName']?.toString() ??
                                        opts?['manager']?.toString() ??
                                        '주문자 없음'),
                                style: const TextStyle(
                    ...
                    Wrap(
                      ...
                      children: [
                        if (opts['teamName'] != null &&
                            (opts['teamName'] as String).isNotEmpty)
                          _miniTag(
                              '팀명: ${opts['teamName']}', AppColors.primary),
                        if (opts['totalCount'] != null)
                          _miniTag('${opts['totalCount']}명', AppColors.info),
                        if (opts['printTypeLabel'] != null)
                          _miniTag(opts['printTypeLabel'] as String,
                              AppColors.success),
                        if (opts['mainColor'] != null &&
                            (opts['mainColor'] as String).isNotEmpty)
                          _miniTag(
                              '색상: ${opts['mainColor']}', AppColors.accent),
                      ],
                    ),
                  ],
                  const SizedBox(height: 10),
                              ...
                              if (newStatus == OrderStatus.shipped && mounted) {
                                ...
                                if (mounted) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                          '주문 ${order.id} 상태: ${newStatus.label} 🔔FCM+알림톡 전송'),
                                      backgroundColor: AppColors.primary,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _detailSection('주문자 정보', [
                            _detailRow(
                                Icons.person_outline, '이름', order.userName),
                            _detailRow(
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      if (item.color.isNotEmpty)
                                        _miniTag(item.color, AppColors.accent),
                                      // 사이즈 (개인주문만, '단체' 값 제외)
                                      if (displaySize != null) ...[
                                        const SizedBox(width: 4),
                                        _miniTag(
                                      ...
                                      Text('${item.quantity}개',
                                          style: const TextStyle(
                                              ...
                                              fontWeight: FontWeight.w600)),
                                      Text(
                                          ' · ${_fmtPrice(item.price * item.quantity)}원',
                                          style: const TextStyle(
                                              fontSize: 12,
                                              color: AppColors.textSecondary)),
                                    ],
                          ...
                          _detailSection('결제 정보', [
                            _detailRow(Icons.payments_outlined, '결제 방법',
                                order.paymentMethod),
                            _detailRow(Icons.local_shipping_outlined, '배송비',
                                '${_fmtPrice(order.shippingFee)}원'),
                            _detailRow(Icons.receipt_outlined, '합계',
                                '${_fmtPrice(order.totalAmount)}원'),
                            if (order.memo != null && order.memo!.isNotEmpty)
                              _detailRow(
                                  Icons.notes_outlined, '메모', order.memo!),
                          ]),
                        ],
                      ),
                    ),
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () async {
                            ...
                            if (confirmed == true && mounted) {
                              await OrderService.updateOrderStatus(
                                  currentOrder.id, OrderStatus.confirmed);
                              if (!mounted) return;
  ...
  void _showDesignRevisionAdminSheet(OrderModel order) {
    ...
    final canChangeColor =
        printType == 0 || printType == 2 || printType == 3 || printType == 4;
    final canChangeTeamName =
        printType == 1 || printType == 2 || printType == 3 || printType == 4;
...
// ignore: unused_local_variable
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        // 요청 내용 확인
                        if ((order.customOptions?['designRevisionRequest']
                                as Map?) !=
                            null) ...[
                          Container(
                            padding: const EdgeInsets.all(12),
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  Text(
                                      '${(order.customOptions!['designRevisionRequest'] as Map)['memo'] ?? '내용 없음'}',
                                      style: const TextStyle(
                                          fontSize: 13, height: 1.4)),
                                  if ((order.customOptions![
                                              'designRevisionRequest']
                                          as Map)['color'] !=
                                      null)
                                    Text(
                                        '요청 색상: ${(order.customOptions!['designRevisionRequest'] as Map)['color']}',
                                        style: const TextStyle(
                                            fontSize: 12,
                                            color: AppColors.textSecondary)),
                                ]),
                        ...
                        Wrap(spacing: 8, runSpacing: 6, children: [
                          if (canChangeColor)
                            _adminChip('색상 변경 가능', AppColors.info),
                          if (canChangeTeamName)
                            _adminChip('단체명 변경 가능', AppColors.success),
                          if (canChangeDesign)
                            _adminChip('디자인 변경 가능', AppColors.primary),
                        ]),
                        const SizedBox(height: 16),
                        // 수정 내용 입력 (선택 사항)
                        const Text('관리자 처리 내용 (선택)',
                            style: TextStyle(
                                fontSize: 13, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 8),
                        TextField(
                            ...
                            children: [
                              ...
                              return GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color:
                                        isSel ? AppColors.info : Colors.white,
                                    borderRadius: BorderRadius.circular(20),
                                    border: Border.all(
                                        color: isSel
                                            ? AppColors.info
                        ...
                        const SizedBox(height: 4),
                        const Text(
                            '수정 완료 후 확정된 최종 디자인 이미지를 첨부해 주세요.\n고객의 추가제작 주문서에 자동으로 표시됩니다.',
                            style: TextStyle(
                                fontSize: 11,
                                color: AppColors.textSecondary,
                                height: 1.5)),
                        ...
                        onPressed: isSubmitting
                            ...
                            : () async {
                                ...
                                try {
                                  ...
                                  final uploadUrl =
                                      await StorageService.uploadBannerImage(
                                    bannerId:
                                        'design_confirmed/${order.id}_${DateTime.now().millisecondsSinceEpoch}',
                                  ...
                                  await db
                                    ...
                                    'designRevisionRequest.respondedAt':
                                        DateTime.now().toIso8601String(),
                                    'designRevisionRequest.adminMemo':
                                        memoCtrl.text.trim(),
                                    ...
                                    'customOptions.designConfirmedImageUrl':
                                        uploadUrl,
                                  });
                                  // TAB8 목록 상태도 '완료'로 업데이트
                                  final reqIdx = _designRequests.indexWhere(
                                      (r) =>
                                          (r['orderId'] ?? r['id']) ==
                                          order.id);
                                  if (reqIdx >= 0) {
                                    setState(() => _designRequests[reqIdx] = {
                                          ..._designRequests[reqIdx],
                                          'status': '완료'
                                        });
                                  }
                                  if (!ctx.mounted) return;
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                        content: Text('디자인 수정 처리가 완료되었습니다.'),
                                        backgroundColor: AppColors.success),
                                  );
                                } catch (e) {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          _whiteTag(
                              '${opts['totalCount'] ?? order.groupCount ?? persons.length}명'),
                          if ((opts['printTypeLabel'] ??
                                  opts['printType'] ??
                                  '')
                              .toString()
                              .isNotEmpty)
                            _whiteTag(
                                (opts['printTypeLabel'] ?? opts['printType'])
                                    .toString()),
                          if ((opts['mainColor'] ?? '').toString().isNotEmpty)
                            _whiteTag('색상: ${opts['mainColor']}'),
                          if ((opts['fabricType'] ?? opts['fabric'] ?? '')
                              .toString()
                              .isNotEmpty)
                            _whiteTag((opts['fabricType'] ?? opts['fabric'])
                                .toString()),
                        ],
                      ),
                    ],
                    ...
                    children: [
                      ...
                      _infoChip(
                          ...
                          order.userAddress.isNotEmpty
                              ? order.userAddress
                              : '-'),
                      _infoChip(Icons.male, '남',
                          '${opts['maleCount'] ?? persons.where((p) => (p as Map)['gender'] == '남').length}명'),
                      _infoChip(Icons.female, '여',
                          '${opts['femaleCount'] ?? persons.where((p) => (p as Map)['gender'] == '여').length}명'),
                      if ((opts['waistbandOption'] ?? opts['waistband'] ?? '')
                          .toString()
                          .isNotEmpty)
                        _infoChip(
                            Icons.style,
                            '허리밴드',
                            (opts['waistbandOption'] ?? opts['waistband'])
                                .toString()),
                    ],
                  ),
                ),
                ...
                if ((currentOrder.customOptions?['designRevisionRequest']
                        as Map?) !=
                    null)
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if ((currentOrder
                            ...
                            null)
                          Text(
                              '• 요청내용: ${(currentOrder.customOptions!['designRevisionRequest'] as Map)['memo']}',
                              style: const TextStyle(
                        ...
                        if ((currentOrder
                            ...
                            null)
                          Text(
                              '• 색상: ${(currentOrder.customOptions!['designRevisionRequest'] as Map)['color']}',
                              style: const TextStyle(
                                  fontSize: 12, color: Color(0xFF5D4037))),
                        const SizedBox(height: 8),
                        SizedBox(
  ...
  // ── 개인 주문 PDF 내보내기 ──
  Future<void> _exportPersonalOrderPdf(OrderModel order) async {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Row(children: [
    ...
    try {
      final now = DateTime.now();
      final bytes =
          await OrderExcelService.generateSelectedOrdersPdf([order], now);
      final dateStr =
      ...
      final fileName = '주문_${safeName}_$dateStr.pdf';
      const mimeType = 'application/pdf';
      ...
      if (kIsWeb) {
        ...
        await SharePlus.instance.share(
          ShareParams(
            files: [XFile(filePath, mimeType: mimeType, name: fileName)],
            subject: '2FIT 주문 ${order.userName} PDF',
            text: fileName,
          ),
        );
      }
...
// ignore: unused_local_variable
  ...
  Widget _buildProductManagement() {
    ...
    bool matchesCategory(p, String filter) {
    ...
    var filtered = _productCategoryFilter == '전체'
        ...
        : allProducts
            .where((p) => matchesCategory(p, _productCategoryFilter))
            .toList();
                ...
                onTap: () async {
                  final ok = await showDialog<bool>(
                    ...
                    builder: (_) => AlertDialog(
                      ...
                      content: Text(
                          '선택한 ${_selectedProductIds.length}개 상품을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.'),
                      actions: [
                        TextButton(
                            onPressed: () => Navigator.pop(context, false),
                            child: const Text('취소')),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          if (p.isNewActive) _miniTag('NEW', AppColors.info),
                          if (p.isSale) _miniTag('SALE', AppColors.error),
                          if (p.isFreeShipping)
                            _miniTag('무료배송', AppColors.success),
                          const Spacer(),
                          // 재고
                          Container(
                            padding: const EdgeInsets.symmetric(
                  ...
                  const PopupMenuItem(
                      ...
                      child: Row(children: [
                        Icon(Icons.copy_all_rounded,
                            size: 16, color: AppColors.info),
                        SizedBox(width: 8),
                        Text('복사', style: TextStyle(color: AppColors.info))
                      ])),
                  const PopupMenuItem(
                      value: 'stock',
                      child: Row(children: [
  ...
  Widget _buildBannerManagement() {
    return StreamBuilder<List<BannerModel>>(
      ...
      builder: (context, snap) {
        if (snap.hasError) {
          return Center(
              child: Text('배너 로드 오류: ${snap.error}',
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.error_outline_rounded,
                    size: 48, color: AppColors.error),
                const SizedBox(height: 12),
                Text('회원 데이터 로드 실패: ${snapshot.error}',
                    textAlign: TextAlign.center,
                    style: const TextStyle(
                        fontSize: 13, color: AppColors.textSecondary)),
                const SizedBox(height: 16),
        ...
        final allSelected = members.isNotEmpty &&
            members.every((m) => _selectedMemberIds
                .contains(m['uid'] as String? ?? m['id'] as String? ?? ''));
                ...
                children: [
                  GestureDetector(
                    onTap: () {
                      setState(() {
                        if (allSelected) {
                          _selectedMemberIds.removeAll(members.map((m) =>
                              m['uid'] as String? ?? m['id'] as String? ?? ''));
                        } else {
                          _selectedMemberIds.addAll(members.map((m) =>
                              m['uid'] as String? ?? m['id'] as String? ?? ''));
                        }
                      });
                    },
                      ...
                      onTap: () async {
                        final ok = await showDialog<bool>(
                          ...
                          builder: (_) => AlertDialog(
                            ...
                            content: Text(
                                '선택한 ${_selectedMemberIds.length}명의 회원 계정을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.'),
                            actions: [
                              TextButton(
                                  onPressed: () =>
                                      Navigator.pop(context, false),
              ...
              child: members.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final grade = m['memberTier'] as String? ??
                            m['grade'] as String? ??
                            '일반';
                            ...
                            child: Row(
                              children: [
                                ...
                                PopupMenuButton<String>(
                                  ...
                                  itemBuilder: (_) => [
                                    const PopupMenuItem(
                                        value: 'view', child: Text('주문 내역')),
                                    const PopupMenuItem(
                                        value: 'grade', child: Text('등급 변경')),
                                    const PopupMenuItem(
                      ...
                      try {
                        ...
                        if (mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                  success ? '✅ 알림이 발송되었습니다' : '⚠️ 알림 발송 실패'),
                              backgroundColor:
              ...
              onPressed: selectedGrade == null
                  ...
                  : () async {
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text('"$selectedGrade" 등급으로 변경 완료'),
                              backgroundColor: AppColors.primary),
                        );
                      }
                    },
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Flexible(
            flex: 2,
            child: Text('${_fmtPrice(order.totalAmount)}원',
                style: const TextStyle(
                    fontSize: 11,
                    fontWeight: FontWeight.w800,
                    color: AppColors.primary),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(20),
            border: Border.all(
                color: val ? ac : AppColors.border, width: val ? 1.5 : 1)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(val ? Icons.check_circle : Icons.circle_outlined,
              size: 14, color: val ? ac : AppColors.textHint),
          const SizedBox(width: 4),
          Text(label,
              style: TextStyle(
                  fontSize: 12,
  ...
  Widget _divider() => const Divider(height: 1, color: AppColors.surfaceGray);
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: useLocalVideo
                                ...
                                : (banner.videoUrl ==
                                        'assets/images/banner_video.mp4'
                          ...
                          child: Text(
                            ...
                            style: TextStyle(
                              ...
                              fontWeight: FontWeight.w700,
                              color: useLocalVideo
                                  ? AppColors.info.withValues(alpha: 0.82)
                                  : (banner.videoUrl ==
                                          'assets/images/banner_video.mp4'
                                      ? AppColors.success
                                          .withValues(alpha: 0.82)
                        ...
                        child: Row(
                          children: [
                            Icon(
                              ...
                              size: 22,
                              color: useLocalVideo
                                  ? AppColors.info.withValues(alpha: 0.70)
                            ...
                            if (useLocalVideo)
                              Icon(Icons.check_circle,
                                  color: AppColors.info.withValues(alpha: 0.70),
                                  size: 20),
                          ],
                        ),
                      ),
                    ...
                    : () async {
                        ...
                        if (pickedImageBytes != null) {
                          final url = await StorageService.uploadBannerImage(
                            bannerId:
                                '${banner.id}_${DateTime.now().millisecondsSinceEpoch}',
                        ...
                        // 갤러리 업로드 동영상 → Firebase Storage
                          ...
                          await for (final progress
                              in StorageService.uploadBannerVideoWithProgress(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text('배너 CTA 버튼 클릭 시 쿠폰 다운로드 팝업이 표시됩니다.',
                            style: TextStyle(
                                fontSize: 11, color: Color(0xFF9C27B0))),
                      ],
                    ),
                  ),
              ]),
              ...
              onPressed: isUploading
                  ...
                  : () async {
                      ...
                      if (pickedImageBytes != null) {
                        final url = await StorageService.uploadBannerImage(
                          bannerId:
                              'banner_new_${DateTime.now().millisecondsSinceEpoch}',
                          imageBytes: pickedImageBytes!,
                        );
                        imageUrl = url ?? '';
                      }
  ...
  Widget _buildScheduleBadge(DateTime? startDate, DateTime? endDate) {
    ...
    if (startDate != null && now.isBefore(startDate)) {
      ...
      textColor = AppColors.error.withValues(alpha: 0.82);
    } else {
      // 현재 노출 중
      final endStr =
          endDate != null ? '~${endDate.month}/${endDate.day}' : '~상시';
      label = '📅 $endStr';
    ...
    showDialog(
      ...
      builder: (ctx) => _ProductFormDialog(
        ...
        onSaved: (product, isEdit) async {
          ...
          try {
            await InventoryService.initProduct(product);
          } catch (_) {}
          if (mounted) setState(() {});
        },
      ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await ProductService.updateStockWithSizes(
                    p.id, newStock, newSizeStocks);
                for (final c in sizeCtrls.values) {
                  c.dispose();
                }
                if (!context.mounted) return;
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                await OrderService.updateOrderStatusWithTracking(
                  ...
                  adminMemo: memoCtrl.text.trim().isEmpty
                      ? null
                      : memoCtrl.text.trim(),
                );
                if (!context.mounted) return;
                ...
                if (targetOrder.id == orderId) {
                  ...
                  if (trackingNum.isNotEmpty &&
                      targetOrder.userPhone.isNotEmpty) {
  ...
  void _showDeleteProductDialog(ProductModel p) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title:
            const Text('상품 삭제', style: TextStyle(fontWeight: FontWeight.w800)),
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          title: Text(
                              ...
                              overflow: TextOverflow.ellipsis),
                          subtitle: Text(
                              '${_fmtPrice(o.totalAmount)}원 · ${_fmtDate(o.createdAt)}',
                              style: const TextStyle(
                                  fontSize: 11,
                                  color: AppColors.textSecondary)),
                        );
  ...
  void _showMemberGradeDialog(Map<String, dynamic> m) {
    ...
    final name = m['name'] as String? ?? '회원';
    String selectedGrade =
        m['memberTier'] as String? ?? m['grade'] as String? ?? '일반';
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text('$name 등급이 $selectedGrade(으)로 변경되었습니다'),
                        backgroundColor: AppColors.primary),
                  );
                }
              },
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          Icon(isBlocked ? Icons.lock_open_rounded : Icons.block_rounded,
              color: isBlocked ? AppColors.success : AppColors.error, size: 20),
          const SizedBox(width: 8),
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                        isBlocked ? '$name 차단이 해제되었습니다' : '$name 계정이 차단되었습니다'),
                    backgroundColor:
  ...
  void _showMemberMemoDialog(Map<String, dynamic> m) {
    ...
    final name = m['name'] as String? ?? '회원';
    final memoCtrl =
        TextEditingController(text: m['adminMemo'] as String? ?? '');
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        ...
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: const Text('적립', style: TextStyle(color: Colors.white)),
            ),
          ],
        ),
      ),
              ...
              if (kIsWeb)
                TextButton.icon(
                  ...
                  icon: Icon(
                    ...
                    size: 14,
                    color: AdminWebNotifier.isGranted
                        ? AppColors.success
                        : AppColors.error,
                  ),
                  label: Text(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SwitchListTile.adaptive(
                      dense: true,
                      contentPadding: EdgeInsets.zero,
                      title: const Text('새 채팅 문의 알림',
                          style: TextStyle(fontSize: 13)),
                      value: _notifyNewChat,
                      activeColor: AppColors.primary,
                      onChanged: (v) {
                        setState(() => _notifyNewChat = v);
                        setDlgState(() {});
                        _saveAdminSettings();
                      },
                    ),
                    SwitchListTile.adaptive(
                      ...
                      contentPadding: EdgeInsets.zero,
                      title:
                          const Text('새 주문 알림', style: TextStyle(fontSize: 13)),
                    ...
                    SwitchListTile.adaptive(
                      dense: true,
                      contentPadding: EdgeInsets.zero,
                      title: const Text('문자(SMS) 알림 연동',
                          style: TextStyle(fontSize: 13)),
                      subtitle: const Text('관리자 전화번호로 문의/주문 알림 발송',
                          style: TextStyle(fontSize: 11)),
                      value: _notifyChatSms,
                      activeColor: AppColors.primary,
                      onChanged: (v) {
                        setState(() => _notifyChatSms = v);
                        setDlgState(() {});
                        _saveAdminSettings();
                      },
                    ),
                    ...
                    Container(
                      ...
                      child: const Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 6),
                          Text('• 브라우저 알림 허용 시 새 문의/주문 즉시 알림',
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFF5D4037))),
                          Text('• 채팅 문의 시 관리자 화면에 실시간 반영',
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFF5D4037))),
                          Text('• 외부 SMS/카카오 연동은 서버 API 설정 필요',
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFF5D4037))),
                          Text('• 카카오채널: @2fit-mall',
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFF5D4037))),
                          SizedBox(height: 4),
                          SelectableText(
    ...
    return Column(
      children: [
              ...
              StreamBuilder<List<ChatRoomModel>>(
                ...
                builder: (_, snap) {
                  final unread = (snap.data ?? [])
                      .where((r) => !r.isCompleted && !r.isBlocked)
        ...
        Container(
          ...
          child: Row(
            children: [
              _chatFilterChip('active', '상담중', Icons.chat_bubble_rounded,
                  AppColors.success),
              const SizedBox(width: 6),
              _chatFilterChip('completed', '완료', Icons.check_circle_rounded,
                  AppColors.textSecondary),
              const SizedBox(width: 6),
              _chatFilterChip(
                  'blocked', '차단', Icons.block_rounded, AppColors.error),
              const SizedBox(width: 6),
              _chatFilterChip(
                  'all', '전체', Icons.list_rounded, AppColors.primary),
            ],
          ),
        ),
        // 알림 배너
          ...
          child: StreamBuilder<List<ChatRoomModel>>(
            ...
            builder: (context, snapshot) {
              ...
              final rooms = allRooms.where((r) {
                if (_chatFilterTab == 'active') {
                  return !r.isCompleted && !r.isBlocked;
                }
                if (_chatFilterTab == 'completed') {
                  return r.isCompleted && !r.isBlocked;
                }
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text(
                        _chatFilterTab == 'completed'
                            ? '완료된 상담이 없습니다'
                            : _chatFilterTab == 'blocked'
                                ? '차단된 사용자가 없습니다'
                                : '아직 채팅 문의가 없습니다',
                        style: const TextStyle(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Icon(icon,
                size: 12, color: isSelected ? color : AppColors.textSecondary),
            const SizedBox(width: 4),
            Text(label,
                style: TextStyle(
                    fontSize: 11,
          ...
          children: [
              ...
              child: Row(
                children: [
                  CircleAvatar(
                      ...
                      child: Text(room.userName.characters.first,
                          style: const TextStyle(
                              fontSize: 12,
                              fontWeight: FontWeight.w700,
                              color: AppColors.primary))),
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (room.isCompleted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text('상담을 재개했습니다.'),
                          backgroundColor: AppColors.success),
                    );
                  } else {
                    await ChatService.completeRoom(room.id,
                        completedBy: 'admin');
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text('상담을 완료 처리했습니다.'),
                          backgroundColor: AppColors.textSecondary),
                    );
                  }
                },
            ...
            ListTile(
              leading: Icon(
                ...
                color: room.isBlocked ? AppColors.success : AppColors.accent,
              ),
              title: Text(room.isBlocked ? '차단 해제' : '사용자 차단'),
              subtitle: room.isBlocked
                  ? null
                  : const Text('차단 시 해당 사용자의 메시지를 받지 않습니다',
                      style: TextStyle(fontSize: 11)),
              onTap: () async {
                Navigator.pop(context);
                if (room.isBlocked) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text('${room.userName} 차단을 해제했습니다.'),
                        backgroundColor: AppColors.success),
                  );
                } else {
                  ...
                  if (confirm == true) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text('${room.userName}님을 차단했습니다.'),
                          backgroundColor: AppColors.error),
                    );
                  }
                }
            ...
            ListTile(
              leading: const Icon(Icons.delete_outline_rounded,
                  color: AppColors.error),
              title:
                  const Text('채팅 삭제', style: TextStyle(color: AppColors.error)),
              subtitle: const Text('모든 대화 내용이 영구 삭제됩니다',
                  style: TextStyle(fontSize: 11)),
              onTap: () async {
                Navigator.pop(context);
                final confirm = await showDialog<bool>(
                  ...
                  builder: (_) => AlertDialog(
                    ...
                    content: Text(
                        '${room.userName}님과의 모든 채팅을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.'),
                    actions: [
                      TextButton(
                          onPressed: () => Navigator.pop(context, false),
                          child: const Text('취소')),
                ...
                if (confirm == true) {
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text('${room.userName}님의 채팅을 삭제했습니다.'),
                          backgroundColor: AppColors.error),
                    );
                  } catch (e) {
                    if (!mounted) return;
                  ...
                  StreamBuilder<bool>(
                    ...
                    builder: (_, snap) {
                      ...
                      return isCompleted
                          ...
                          : GestureDetector(
                              onTap: () async {
                                await ChatService.completeRoom(roomId,
                                    completedBy: 'admin');
                                if (!context.mounted) return;
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(session['user'] as String,
                            style: const TextStyle(
                                fontSize: 14, fontWeight: FontWeight.w700),
                            overflow: TextOverflow.ellipsis),
                        const Text('한국어로 표시 중',
                            style: TextStyle(
                                fontSize: 10, color: Color(0xFFFF8F00))),
                      ],
                    ),
                  ),
                  Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: products
                        ...
                        .toList(),
                    onChanged: (val) =>
                        setState(() => _sectionSelectedProductId = val),
                  ),
                ),
              ),
            ],
              ...
              onPressed: isUploading
                  ...
                  : () async {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text('섹션 "$title" 이 추가되었습니다'),
                              backgroundColor: AppColors.primary),
                        );
                      }
                    },
  ...
  Widget _buildColorManagement() {
    final filtered = _colorCategoryFilter == '전체'
        ? _colorItems
        : _colorItems
            .where((c) => c['category'] == _colorCategoryFilter)
            .toList();
    final allSelected = filtered.isNotEmpty &&
      ...
      children: [
        ...
        if (anySelected)
          Container(
            ...
            child: Row(
              children: [
                Text('${_selectedColorIds.length}개 선택됨',
                    style: const TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w700,
                        color: AppColors.accent)),
                        ...
                        child: Row(
                          children: [
                            Checkbox(
                              ...
                              onChanged: (v) => setState(() {
                                if (v == true) {
                                  _selectedColorIds.addAll(
                                      filtered.map((c) => c['id'] as String));
                                } else {
                                  _selectedColorIds.removeAll(
                                      filtered.map((c) => c['id'] as String));
                                }
                              }),
                              activeColor: AppColors.primary,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Switch(
                            ...
                            thumbColor:
                                const WidgetStatePropertyAll(AppColors.success),
                          ),
                        ],
                      ),
                    ),
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (name.isEmpty || hex.length < 4) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('색상 이름과 HEX 코드를 입력해주세요'),
                        backgroundColor: AppColors.error),
                  );
                  return;
                }
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                DropdownButtonFormField<String>(
                  value: ['기본색', '포인트색', '시즌색'].contains(category)
                      ? category
                      : '기본색',
                  decoration: InputDecoration(
                    border: OutlineInputBorder(
                ...
                Row(
                  children: [
                    Expanded(
                      child: _switchTile(
                          '이미지 연결', hasImage, (v) => setS(() => hasImage = v)),
                    ),
                    Expanded(
                      child: _switchTile('버튼카드', buttonCard,
                          (v) => setS(() => buttonCard = v)),
                    ),
                    Expanded(
                      child: _switchTile(
                          '활성화', active, (v) => setS(() => active = v)),
                    ),
                  ],
                ),
              ],
  ...
  Widget _buildDesignRequests() {
    final filtered = _designRequestFilter == '전체'
        ? _designRequests
        : _designRequests
            .where((r) => r['status'] == _designRequestFilter)
            .toList();
    final allSelected = filtered.isNotEmpty &&
        filtered.every(
            (r) => _selectedDesignRequestIds.contains(r['id'] as String));
            ...
            child: Row(
              children: [
                ...
                TextButton.icon(
                  onPressed: () {
                    setState(() {
                      _designRequests.removeWhere((r) =>
                          _selectedDesignRequestIds
                              .contains(r['id'] as String));
                      _selectedDesignRequestIds.clear();
                    });
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text('선택된 요청이 삭제되었습니다'),
                          backgroundColor: AppColors.error),
                    );
                  },
                  icon: const Icon(Icons.delete_rounded,
                ...
                TextButton(
                  onPressed: () =>
                      setState(() => _selectedDesignRequestIds.clear()),
                  child: const Text('취소',
                      style: TextStyle(color: AppColors.textSecondary)),
                ),
              ],
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        _designRequestFilter == '전체'
                            ? '디자인 수정 요청이 없습니다'
                            : '"$_designRequestFilter" 상태의 요청이 없습니다',
                        style: const TextStyle(
                            fontSize: 15, color: AppColors.textSecondary),
                      ),
                    ],
                        ...
                        child: Row(
                          children: [
                            Checkbox(
                              ...
                              onChanged: (v) => setState(() {
                                if (v == true) {
                                  _selectedDesignRequestIds.addAll(
                                      filtered.map((r) => r['id'] as String));
                                } else {
                                  _selectedDesignRequestIds.removeAll(
                                      filtered.map((r) => r['id'] as String));
                                }
                              }),
                              activeColor: AppColors.primary,
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Builder(builder: (_) {
                                        final colorName =
                                            req['colorName'] as String? ?? '';
                                        final adjustedHex =
                                            req['adjustedColorHex']
                                                    as String? ??
                                                '';
                                        final teamName =
                                            req['teamName'] as String? ?? '';
                                        final fabricName =
                                            req['fabricName'] as String? ?? '';
                                        final printType =
                                            req['printType'] as String? ?? '';
                                        final memo =
                                            req['memo'] as String? ?? '';
                                        final personChanges =
                                            req['personChanges']
                                        ...
                                        return Column(
                                          ...
                                          children: [
                                            ...
                                            if (fabricName.isNotEmpty)
                                              _drInfoRow(Icons.texture_rounded,
                                                  '원단', fabricName, null),
                                            // 인쇄타입
                                            if (printType.isNotEmpty)
                                              _drInfoRow(Icons.print_rounded,
                                                  '인쇄타입', printType, null),
                                            // 단체명
                                            if (teamName.isNotEmpty)
                                              _drInfoRow(Icons.groups_outlined,
                                                  '단체명', teamName, null),
                                            // 인원별 사이즈 변경
                                                  ...
                                                  children: [
                                                    ...
                                                    ...personChanges
                                                      ...
                                                      final after = m['after']
                                                          ...
                                                          '';
                                                      return Text(
                                                        '$idx번${name.isNotEmpty ? " $name" : ""}: $before → $after',
                                                        style: const TextStyle(
                                                            fontSize: 11,
                                                            color: Color(
                                                                0xFF1A1A1A),
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const SizedBox(width: 4),
                                  Expanded(
                                      child: Text(req['adminNote'] as String,
                                          style: const TextStyle(
                                              fontSize: 11,
                                ...
                                if (status == '대기중')
                                  Expanded(
                                      child: _drActionBtn(
                                          ...
                                          AppColors.info,
                                          () async =>
                                              _updateDesignRequestStatus(
                                                  req, '처리중'))),
                                if (status == '처리중') ...[
                                  Expanded(
                                      child: _drActionBtn(
                                          '완료 처리', AppColors.success, () async {
                                    ...
                                    try {
                                      ...
                                      final order =
                                          OrderService.parseOrderFromFirestore(
                                              doc.data()!,
                                              docId: orderId);
  ...
  Widget _drInfoRow(
          IconData icon, String label, String value, Color? dotColor) =>
      Padding(
  ...
  Widget _drActionBtn(
          String label, Color color, Future<void> Function() onTap) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 7),
...
class _AdminSectionCardState extends State<_AdminSectionCard> {
  ...
  Future<void> _pickImages() async {
    ...
    try {
      ...
      await context
          .read<ProductProvider>()
          .updateSectionImages(widget.product.id, widget.sectionKey, current);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  void _deleteAll() async {
    ...
    if (ok == true) {
      await context
          .read<ProductProvider>()
          .updateSectionImages(widget.product.id, widget.sectionKey, []);
      if (!mounted) return;
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                ...
                children: [
                  ...
                  Icon(
                    _expanded
                        ? Icons.keyboard_arrow_up_rounded
                        : Icons.keyboard_arrow_down_rounded,
                    color: _expanded ? Colors.white70 : AppColors.textSecondary,
                  ),
                ],
              ),
            ),
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                // 아래로
                                IconButton(
                                  onPressed: idx < imgs.length - 1
                                      ? () => _moveDown(idx)
                                      : null,
                                  icon: Icon(Icons.keyboard_arrow_down_rounded,
                                      color: idx < imgs.length - 1
                                          ? AppColors.textSecondary
                                          : AppColors.border,
                                      size: 20),
                                  padding: const EdgeInsets.all(4),
...
class _ProductFormDialogState extends State<_ProductFormDialog> {
  ...
  void _onNameChanged(String val) {
    ...
    Future.delayed(const Duration(milliseconds: 1200), () {
      ...
      final now = DateTime.now();
      if (_lastTyped != null &&
          now.difference(_lastTyped!).inMilliseconds >= 1200) {
        final name = val.trim();
  ...
  static const String _singletSetADesc = '2FIT 싱글렛 A타입 유니폼 세트입니다.\n'
  ...
  List<String> get _images =>
      _imageItems.map((e) => e['url'] as String).toList();
  bool _isUploading = false;
...
// ignore: unused_field
  final bool _autoSaved = false; // 마지막 자동저장 성공 표시
  ...
  Future<void> _pickImages() async {
    ...
    try {
      ...
      for (int i = 0; i < files.length; i++) {
        if (!mounted) break;
        setState(() =>
            _uploadStatus = 'Firebase 업로드 중... (${i + 1}/${files.length})');
        try {
          final bytes = await files[i].readAsBytes();
          ...
          if (url.isNotEmpty && mounted) {
            ...
            if (targetIdx < _imageItems.length) {
              setState(() =>
                  _imageItems[targetIdx] = {'url': url, 'pending': false});
            }
            success++;
          }
        } catch (e) {
  ...
  void _addImageByUrl() {
    ...
    if (url.isEmpty) return;
    setState(() {
      _imageItems.add({'url': url, 'pending': false});
    });
    _urlCtrl.clear();
  }
  ...
  void _moveUp(int i) {
    if (i == 0) return;
    setState(() {
      final tmp = _imageItems[i];
      _imageItems[i] = _imageItems[i - 1];
      _imageItems[i - 1] = tmp;
    });
  }
  ...
  Future<void> _save() async {
    ...
    if (_isUploading) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('이미지 업로드 중입니다. 완료 후 저장해주세요.')));
      return;
    }
    ...
    if (descTranslations.isEmpty && productDesc.isNotEmpty) {
      descTranslations =
          await TranslationService.translateLongText(productDesc);
    ...
    final product = ProductModel(
      ...
      price: price,
      originalPrice:
          (origPrice != null && origPrice > price) ? origPrice : null,
      ...
      stockCount: _selectedColors.isNotEmpty && _selectedSizes.isNotEmpty
          ...
          : _sizeStockCtrls.isEmpty
              ...
              : _sizeStockCtrls.entries
                  ...
                  .fold(0,
                      (total, e) => total + (int.tryParse(e.value.text) ?? 0)),
              ...
              child: Column(
                  ...
                  children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_nameTranslations['en'] != null)
                              _translationRow(
                                  '🇺🇸 EN', _nameTranslations['en']!),
                            if (_nameTranslations['ja'] != null)
                              _translationRow(
                                  '🇯🇵 JA', _nameTranslations['ja']!),
                            if (_nameTranslations['zh'] != null)
                              _translationRow(
                                  '🇨🇳 ZH', _nameTranslations['zh']!),
                            if (_nameTranslations['mn'] != null)
                              _translationRow(
                                  '🇲🇳 MN', _nameTranslations['mn']!),
                          ],
                        ),
                      ),
                    ],
                    ...
                    Row(children: [
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              value: _currentSubCats.contains(_selSubCat)
                                  ? _selSubCat
                                  : (_currentSubCats.isNotEmpty
                                      ? _currentSubCats.first
                                      : null),
                              isExpanded: true,
                              padding:
                                  const EdgeInsets.symmetric(horizontal: 14),
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              () {
                                final isSingletSet = _selCat == '세트' ||
                                    _selSubCat.contains('싱글렛세트');
                                final isSingletTop = !isSingletSet &&
                                    (_selCat == '상의' ||
                                        _selSubCat.contains('싱글렛'));
                                final isTaiz = _selSubCat.contains('타이즈') ||
                                    _selCat == '하의';
                                if (isSingletSet) {
                                  return '자동: 상의 폴리에스터 92%/라이크라 8% + 하의 나일론 75%/라이크라 25%';
                                }
                                if (isSingletTop) {
                                  return '자동: 폴리에스터 92% / 라이크라 8%';
                                }
                    ...
                    if (_selCat == '하의' ||
                        _selCat == '세트' ||
                        _selSubCat.contains('싱글렛세트') ||
                        _selSubCat.contains('타이즈')) ...[
                      _lbl('하의길이 (직접 입력 시 기본값 덮어씀)'),
                      TextField(
                        controller: _bottomLengthCtrl,
                        decoration: InputDecoration(
                          hintText:
                              '비워두면 자동 표시 (싱글렛세트: 남5부/여2.5부, 타이즈: 서브카테고리 값)',
                          hintStyle: const TextStyle(
                              fontSize: 11, color: AppColors.textHint),
                          filled: true,
                          fillColor: AppColors.surfaceGray,
                    ...
                    Wrap(spacing: 8, runSpacing: 6, children: [
                      ...
                      _chip('단체전용', _isGroupOnly, (v) {
                        ...
                        _scheduleAutoSave();
                      }, ac: AppColors.primary),
                      _chip('기성품', _isReadyMade, (v) {
                        setState(() {
                          _isReadyMade = v;
  ...
  Widget _buildSizeSelector() {
    ...
    void selectPreset(List<String> preset) {
      setState(() {
        ...
        final removed = _sizeStockCtrls.keys
            .where((s) => !_selectedSizes.contains(s))
            .toList();
        for (final s in removed) {
          _sizeStockCtrls[s]?.dispose();
          _sizeStockCtrls.remove(s);
        }
        for (final s in preset) {
          _sizeStockCtrls[s] ??= TextEditingController(text: '100');
        }
        final ordered = [
          ..._adultSizeOptions.where(_selectedSizes.contains),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            _presetBtn('성인 기본', ['S', 'M', 'L', 'XL'], AppColors.primary,
                selectPreset),
            const SizedBox(width: 6),
            _presetBtn('성인 전체', ['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL'],
                AppColors.primary, selectPreset),
            const SizedBox(width: 6),
            _presetBtn(
                '주니어 전체', _juniorSizeOptions, AppColors.info, selectPreset),
            const SizedBox(width: 6),
            _presetBtn('전체 해제', [], AppColors.textSecondary, selectPreset),
          ]),
          const SizedBox(height: 12),
          ...
          Wrap(
            spacing: 7,
            runSpacing: 7,
            children: _adultSizeOptions
                .map((s) => sizeChip(s, AppColors.primary))
                .toList(),
          ),
          const SizedBox(height: 12),

          // ── 주니어 사이즈
          groupLabel('주니어', Icons.child_care_rounded, AppColors.info),
          Wrap(
            spacing: 7,
            runSpacing: 7,
            children: _juniorSizeOptions
                .map((s) => sizeChip(s, AppColors.info))
                .toList(),
          ),
          const SizedBox(height: 12),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    _selectedSizes.isEmpty ? '선택된 사이즈 없음' : _sizesCtrl.text,
                    style: TextStyle(
                      fontSize: 12,
            ...
            children: orderedSizes.map<Widget>((s) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: isSoldOut ? AppColors.error : Colors.white,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(
                      color: isSoldOut ? AppColors.error : AppColors.border,
  ...
  // ── 사이즈별 재고 컨트롤러 초기화
  void _initSizeStockCtrls(
      List<String> sizes, Map<String, int> existingStocks) {
    for (final c in _sizeStockCtrls.values) {
      c.dispose();
    }
  ...
  Widget _buildSizeStockEditor() {
    ...
    // 제거된 사이즈 컨트롤러 정리
    final removed =
        _sizeStockCtrls.keys.where((s) => !_selectedSizes.contains(s)).toList();
    for (final s in removed) {
      _sizeStockCtrls[s]?.dispose();
      _sizeStockCtrls.remove(s);
    }
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('"$_selCat" 카테고리에 추가할 하위 항목을 입력하세요.',
                  style: const TextStyle(
                      fontSize: 12, color: AppColors.textSecondary)),
              const SizedBox(height: 10),
              TextField(
  ...
  Widget _chip(String label, bool val, ValueChanged<bool>? onChanged,
          {Color? ac}) =>
      FilterChip(
        label: Text(label,
            style: TextStyle(
                fontSize: 12,
                color: val ? Colors.white : AppColors.textSecondary)),
        selected: val,
        onSelected: onChanged,
        selectedColor: ac ?? AppColors.primary,
        backgroundColor:
            onChanged == null ? const Color(0xFFE8D5F5) : AppColors.surfaceGray,
  ...
  Widget _newChip() {
    ...
    final expires = _newExpiresAt ??
        (widget.existing?.createdAt ?? DateTime.now())
            .add(const Duration(days: 60));
    final expiresLabel =
        _isNew ? '신상품 · ${expires.month}/${expires.day}까지' : '신상품';
    return FilterChip(
      label: Text(
        expiresLabel,
        style: TextStyle(
            fontSize: 12,
            color: _isNew ? Colors.white : AppColors.textSecondary),
      ),
      selected: _isNew,
...
class _NoticeManagementTabState extends State<_NoticeManagementTab> {
  ...
  Future<void> _openEditor({NoticeModel? existing}) async {
    ...
    const themes = [
      ...
      {'id': 'event', 'label': '🎉 이벤트', 'color': AppColors.primary},
      {'id': 'delivery', 'label': '🚚 배송안내', 'color': AppColors.info},
      {'id': 'warning', 'label': '⚠️ 주의사항', 'color': AppColors.accent},
      {'id': 'update', 'label': '✅ 업데이트', 'color': AppColors.success},
                  ...
                  if (autoImage) ...[
                    ...
                    Builder(builder: (_) {
                      final previewTheme = selectedTheme == 'auto'
                          ? NoticeThemeHelper.detectTheme(
                              titleCtrl.text, contentCtrl.text)
                          : selectedTheme;
                      final emoji =
                          NoticeThemeHelper.themeEmoji[previewTheme] ?? '📢';
                      final label =
                          NoticeThemeHelper.themeLabel[previewTheme] ?? '일반공지';
                      final autoUrl =
                          NoticeThemeHelper.autoImageUrl(previewTheme);
                      return Container(
                        padding: const EdgeInsets.all(12),
                        decoration: BoxDecoration(
                            ...
                            child: ElevatedButton.icon(
                              onPressed: isUploading
                                  ...
                                  : () async {
                                      ...
                                      final noticeId = existing?.id ??
                                          'notice_${DateTime.now().millisecondsSinceEpoch}';
                                      await for (final p in StorageService
                                          .uploadNoticeImageWithProgress(
                                        noticeId: noticeId,
                                        imageBytes: pickedImageBytes!,
                                        fileName:
                                            pickedFileName ?? 'notice.jpg',
                                        onComplete: (url) {
                                          setD(() {
                                            uploadedUrl = url;
                                            imageCtrl.text = url;
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                final contentChanged =
                    existing != null && existing.contentKo != content;
                final notice = NoticeModel(
                  id: existing?.id ??
                      'notice_${DateTime.now().millisecondsSinceEpoch}',
                  titleKo: title,
                  contentKo: content,
                  titleTranslations: titleChanged
                      ? const {}
                      : (existing?.titleTranslations ?? const {}),
                  contentTranslations: contentChanged
                      ? const {}
                      : (existing?.contentTranslations ?? const {}),
                  isActive: isActive,
                  ...
                  OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(
                      foregroundColor: isActive
                          ...
                          : AppColors.success,
                      side: BorderSide(
                        color: isActive ? AppColors.border : AppColors.success,
                      ),
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12, vertical: 6),
                      shape: RoundedRectangleBorder(
...
class _CategoryManagementTabState extends State<_CategoryManagementTab> {
  ...
  Future<void> _loadData() async {
    ...
    if (mounted) {
      setState(() {
        _loading = false;
        _selectedMain ??= CategoryService.mainCategories.firstOrNull;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final subCats = _selectedMain != null
        ? CategoryService.subCatsFor(_selectedMain!)
        : <String>[];
  ...
  Future<void> _showAddMainCatDialog() async {
    ...
    if (CategoryService.mainCategories.contains(result)) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('이미 존재합니다: $result')));
      return;
    }
    try {
      await CategoryService.addMainCategory(result);
  ...
  Future<void> _showAddSubCatDialog() async {
    ...
    final result = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(
              child: Text('"$_selectedMain" 하위 카테고리 추가',
                  style: const TextStyle(
                      fontSize: 16, fontWeight: FontWeight.w700))),
        ]),
        content: SizedBox(
          ...
          child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('"$_selectedMain"에 추가할 하위 항목을 입력하세요.',
                    style: const TextStyle(
                        fontSize: 12, color: AppColors.textSecondary)),
                const SizedBox(height: 10),
                TextField(
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('"$_selectedMain" → "$result" 하위 카테고리 추가됨'),
              backgroundColor: const Color(0xFF3F51B5)),
        );
      }
    } catch (e) {
  ...
  Future<void> _deleteMainCat(String cat) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          if (_selectedMain == cat) {
            _selectedMain = CategoryService.mainCategories.firstOrNull;
          }
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('카테고리 "$cat" 삭제됨'),
              backgroundColor: AppColors.error),
        );
      }
    } catch (e) {
