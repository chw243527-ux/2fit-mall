...
class _GroupOrderTestScreenState extends State<GroupOrderTestScreen> {
  ...
  // ── 더미 persons 생성 ──
  List<Map<String, dynamic>> _makeDummyPersons(
    int count, {
    String? maleLength,
    String? femaleLength,
  }) {
  ...
  Future<void> _submit({
    ...
    try {
      ...
      final productName =
          orderType == 'personal' ? (product?.name ?? '테스트 상품') : '단체주문 테스트 상품';
      ...
      final persons = orderType == 'group'
          ? _makeDummyPersons(count,
              maleLength: maleLength, femaleLength: femaleLength)
          : <Map<String, dynamic>>[];

      final customOptions = orderType == 'group'
          ? <String, dynamic>{
              'orderType': 'group',
              'isTest': true,
              'printType': printType,
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    // ── 프리셋 ──
                    _sectionTitle(context.loc.t('빠른_프리셋', '빠른 프리셋'),
                    ...
                    // ── 커스텀 ──
                    _sectionTitle(context.loc.t('커스텀_주문_생성', '커스텀 주문 생성'),
                        Icons.tune_rounded),
                    const SizedBox(height: 8),
                    _customPanel(),
                    const SizedBox(height: 20),
                    ...
                    if (_results.isNotEmpty) ...[
                      _sectionTitle(context.loc.t('결과_로그', '결과 로그'),
  ...
  Widget _userBadge(UserModel? user) => Container(
        ...
        decoration: BoxDecoration(
          color:
              user != null ? const Color(0xFFE8F5E9) : const Color(0xFFFFF3E0),
        ...
        child: Row(children: [
          Icon(
              user != null ? Icons.person_rounded : Icons.warning_amber_rounded,
              size: 16,
              color: user != null ? AppColors.success : AppColors.warning),
              ...
              Expanded(
                child: Column(
                    ...
                    children: [
                      ...
                      Row(children: [
                        _chip(
                            p.orderType == 'personal'
                                ? context.loc.t('기성품', '기성품')
                                : context.loc.t('단체', '단체'),
                            p.color),
                        const SizedBox(width: 4),
                        _chip(
                            '${p.count}${p.orderType == "personal" ? "개" : "인"}',
                            AppColors.info),
                        const SizedBox(width: 4),
  ...
  Widget _customPanel() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            _toggleBtn(
                context.loc.t('기성품_personal', '기성품 (personal)'),
                _customOrderType == 'personal',
                () => setState(() {
                      _customOrderType = 'personal';
                      _customCount = 1;
                    })),
            ...
            _toggleBtn(
                context.loc.t('단체주문_group', '단체주문 (group)'),
                _customOrderType == 'group',
                () => setState(() {
                      _customOrderType = 'group';
                      _customCount = 5;
                    })),
          ...
          Row(children: [
            Expanded(
                child: _field(
                    context.loc.t('팀명_구매자명', '팀명 / 구매자명'), _teamNameCtrl)),
            const SizedBox(width: 8),
          ...
          Row(children: [
            Expanded(
                child: _field(context.loc.t('연락처', '연락처'), _phoneCtrl,
                    keyboard: TextInputType.phone)),
            const SizedBox(width: 8),
            Expanded(
                child: _field(context.loc.t('이메일', '이메일'), _emailCtrl,
