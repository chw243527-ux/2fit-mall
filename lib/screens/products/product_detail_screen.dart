...
2FIT MALL에서 최고의 스포츠웨어를 만나보세요!''';
  ...
  void _showBuyNowSheet(ProductModel product) {
    ...
    cart.addItem(product, size, color, quantity: _quantity, extraPrice: extra);
    Navigator.pushNamed(context, '/checkout');
  }

  // ─── 기성품 장바구니 시트 열기 (사이즈 미선택 시) ───
  void _showBottomLengthSheet(ProductModel product) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _QuickSizeSelectSheet(
        product: product,
        isBuyNow: false,
        onConfirm: (size, qty) {
          final autoColor =
              product.colors.isNotEmpty ? product.colors.first : '기본';
          final extra = _calcExtraForColor(autoColor);
          final cart = context.read<CartProvider>();
          cart.addItem(product, size, autoColor, quantity: qty, extraPrice: extra);
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Row(children: [
  ...
  void _addToCart(ProductModel product) {
    ...
    final color = _selectedColor ?? '기본';
    _doAddToCart(product, _selectedSize!, color, _selectedBottomLength, _quantity);
  }
...
class _ReadyMadePurchaseSheetState extends State<_ReadyMadePurchaseSheet> {
  AppLocalizations get loc => context.watch<LanguageProvider>().loc;
  AppLanguage get _lang => context.watch<LanguageProvider>().language;
  // 0: 싱글렛 기장선택 or 싱글렛세트 성별선택  /  1: 사이즈·컬러·수량
  int _step = 0;
  ...
  int _quantity = 1;
  late List<String> _lengths;

  // 싱글렛세트 성별 ('남' / '여')
  String? _singletSetGender;

  // 싱글렛 단품(상의>싱글렛A/B타입) + 타이즈/하의 카테고리 모두 기장 선택
  bool get _needsLengthStep =>
              // ignore: dead_null_aware_expression
      AppConstants.bottomLengthCategories.contains(widget.product.subCategory ?? '') ||
      _isTaizProduct;

  // 타이즈/하의 카테고리 여부 (싱글렛세트 제외)
  bool get _isTaizProduct =>
      !_isSingletSet && (
  ...
  // 싱글렛세트는 성별 선택 (세트 카테고리 > 싱글렛 A타입세트, 구버전 포함)
  ...
  // 성별 → 하의길이 자동: 남=5부, 여=2.5부
  String _autoLengthForGender(String gender) => gender == '남' ? '5부' : '2.5부';

  @override
  void initState() {
    super.initState();
    _lengths = AppConstants.bottomLengths
        .map((m) => m['label'] ?? '')
        .where((s) => s.isNotEmpty)
        .toList();
    _selectedSize = widget.initialSize;
    _selectedColor = widget.initialColor;
    // 싱글렛세트, 싱글렛(단품) 모두 아니면 step 바로 1로
    if (!_needsLengthStep && !_isSingletSet) {
      _step = 1;
      _selectedLength = '-';
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: _step == 0
          ? (_isSingletSet ? _buildSingletSetGenderStep() : _buildStep1())
          : _buildStep2(),
    );
  }

  // ── 싱글렛세트 성별 선택 (남=5부 고정 / 여=2.5부 고정) ──────────
  Widget _buildSingletSetGenderStep() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        const SizedBox(height: 20),
        Text(context.watch<LanguageProvider>().loc.selectGender,
            style: const TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w900,
                color: Color(0xFF1A1A1A))),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 5),
                Expanded(
                  child: Text(loc.productDeliveryTimeNote,
                      style:
        ...
        Container(
          ...
          child: const Row(children: [
            ...
            Expanded(
              child: Text(
                '싱글렛세트 하의 기장: 남성 5부 고정 · 여성 2.5부 고정',
        ...
        Row(
          children: [
            _genderBtn('남', loc.maleBottomFixed),
            const SizedBox(width: 12),
            _genderBtn('여', loc.femaleBottomFixed),
          ],
        ),
        const SizedBox(height: 20),
        ...
        SizedBox(
          ...
          height: 52,
          child: ElevatedButton(
            onPressed: _singletSetGender == null
                ? null
                : () {
                    final len = _autoLengthForGender(_singletSetGender!);
                    setState(() {
                      _selectedLength = len;
                      _step = 1;
                    });
            ...
            child: Text(
              _singletSetGender == null
                  ? '성별을 선택해주세요'
                  : '다음 단계  ·  하의 ${_autoLengthForGender(_singletSetGender!)} 확정',
              style: const TextStyle(
                  fontSize: 15,
                  fontWeight: FontWeight.w800,
                  color: Colors.white),
  ...
  Widget _genderBtn(String gender, String desc) {
    final isSel = _singletSetGender == gender;
    return Expanded(
      child: GestureDetector(
        onTap: () => setState(() => _singletSetGender = gender),
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 150),
          ...
          child: Column(children: [
            Icon(
              gender == '남' ? Icons.male_rounded : Icons.female_rounded,
              size: 32,
              color: isSel ? Colors.white : const Color(0xFF888888),
            ),
            const SizedBox(height: 6),
            Text(
              gender == '남' ? loc.male : loc.female,
              style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.w800,
            ...
            if (isSel) ...[
              ...
              Container(
                ...
                child: Text(
                  '하의 ${_autoLengthForGender(gender)} 자동선택',
                  style: const TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.w700,
                      color: Colors.white),
  ...
  Widget _buildStep1() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        Text(
          _isTaizProduct ? '하의 길이 선택' : 'STEP 1 · 하의 기장 선택',
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              const Row(children: [
                Icon(Icons.local_shipping_outlined, size: 14, color: Color(0xFF1565C0)),
                SizedBox(width: 5),
                Expanded(
                  child: Text('2~3일 이내 배송 (재고에 따라 기간이 늘어날 수 있음)',
        ...
        ..._lengths.map((len) {
          ...
          final isSel = _selectedLength == len;
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: isSel ? const Color(0xFF1A1A1A) : const Color(0xFFE8E8E8)),
              ),
              child: Row(
                children: [
                  Text(len, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: isSel ? Colors.white : const Color(0xFF1A1A1A))),
                  if (_isTaizProduct && lengthDescMap.containsKey(len)) ...[
                    const SizedBox(width: 8),
                    Text(
                      lengthDescMap[len]!,
                      style: TextStyle(
                        fontSize: 12,
                        color: isSel ? Colors.white.withValues(alpha: 0.75) : const Color(0xFF999999),
                      ),
                    ),
                  ],
                  const Spacer(),
        ...
        SizedBox(
          ...
          height: 52,
          child: ElevatedButton(
            onPressed: _selectedLength == null ? null : () => setState(() => _step = 1),
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFF1A1A1A),
              disabledBackgroundColor: const Color(0xFFCCCCCC),
            ...
            child: Text(loc.nextStepLabel, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Colors.white)),
          ),
        ),
      ],
    );
  ...
  Widget _buildStep2() {
    final sizes = widget.product.sizes.isNotEmpty ? widget.product.sizes : AppConstants.adultSizes;
    // 골지 텍스처: 싱글렛세트 또는 타이즈(하의) 상품에만 적용
    final isBottom = _isSingletSet ||
        widget.product.category == '하의' ||
        widget.product.subCategory == '타이즈' ||
        widget.product.name.contains('타이즈');
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  _isSingletSet
                      ? loc.sizeColorGenderLabel(_singletSetGender == '남' ? loc.male : loc.female, _selectedLength ?? '')
                      : _needsLengthStep
                          ? loc.step2SizeColorLabel(_selectedLength ?? '')
                          : loc.readyMadeProductBuy,
                  softWrap: true,
                ),
              ),
            ],
          ...
          // 사이즈
          Text(loc.sizeLabel, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          Wrap(
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: isSel ? const Color(0xFF1A1A1A) : const Color(0xFFE0E0E0)),
                  ),
                  child: Text(s, style: TextStyle(fontWeight: FontWeight.w700, color: isSel ? Colors.white : const Color(0xFF1A1A1A))),
                ),
              );
            }).toList(),
          ),
          ...
          // ── 무게 선택 (80g / 90g) ──
          Text(loc.weightSelectTitle, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          Row(
              ...
              return Expanded(
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Column(
                      children: [
                        Text(w, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: isSel ? Colors.white : const Color(0xFF1A1A1A))),
                        const SizedBox(height: 2),
                        Text(w == '80g' ? loc.productLightCool : loc.productThickFirm,
                            style: TextStyle(fontSize: 10, color: isSel ? Colors.white70 : const Color(0xFF888888))),
                      ],
                    ),
                  ),
                ),
          ...
          SizedBox(
            ...
            height: 52,
            child: ElevatedButton(
              onPressed: _canConfirm ? () => widget.onConfirm(_selectedLength ?? '-', _selectedSize!, _selectedColor!, _quantity) : null,
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF1A1A1A),
                disabledBackgroundColor: const Color(0xFFCCCCCC),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
  ...
  // ignore: unused_element
  String _getLengthDescStatic(String label) {
    switch (label) {
      case '9부': return '발목 바로 위까지 오는 풀 레깅스 스타일';
      case '5부': return '무릎 약간 위, 가장 범용적인 기장';
      case '4부': return '무릎 바로 위, 활동적인 스타일';
...
class _ColorSelectionWidgetState extends State<_ColorSelectionWidget> {
  ...
  @override
  Widget build(BuildContext context) {
    final colors = widget.isBottomCategory ? _palette : _defaultColors;
    ...
    return Column(
      ...
      children: [
          ...
          children: [
            ...
            if (widget.isBottomCategory)
              Container(
                ...
                child: Text(loc.productColors19, style: const TextStyle(fontSize: 10, color: Color(0xFF333333), fontWeight: FontWeight.w700)),
              )
            else
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                ...
                child: Text(loc.blackNavyLabel, style: const TextStyle(fontSize: 10, color: Color(0xFF1A237E), fontWeight: FontWeight.w700)),
              ),
            if (widget.isBottomCategory) ...[
              const SizedBox(width: 6),
              Container(
                ...
                child: Text(loc.productKPPFree, style: const TextStyle(fontSize: 9, color: Color(0xFF2E7D32), fontWeight: FontWeight.w700)),
              ),
              const SizedBox(width: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                ...
                child: Text(loc.otherColorFee, style: const TextStyle(fontSize: 9, color: Color(0xFFD84315), fontWeight: FontWeight.w700)),
              ),
            ],
          ],
        ),
        ...
        GridView.builder(
          ...
          itemCount: colors.length + (widget.isBottomCategory ? 1 : 0),
          itemBuilder: (_, idx) {
            // 마지막 항목: 직접 입력 버튼
            if (widget.isBottomCategory && idx == colors.length) {
              return GestureDetector(
                ...
                child: Column(
                  ...
                  children: [
                    AnimatedContainer(
                      ...
                      width: 52, height: 52,
                      decoration: BoxDecoration(
                        color: _showCustomInput || _isCustomSelected
                            ? const Color(0xFF1A1A1A)
                            : const Color(0xFFF0F0F0),
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(
                    ...
                    const SizedBox(height: 3),
                    Text(loc.directInputLabel,
                        style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w700, color: Color(0xFF555555), height: 1.2),
                        textAlign: TextAlign.center),
                  ],
                ),
              );
            ...
            final c = colors[idx];
            final sel = widget.selectedColor == c['nameEn'];
            final chipColor = Color(c['hex'] as int);
            final isFree = _isColorFree(c);
            final isLight = chipColor.computeLuminance() > 0.5;
            return GestureDetector(
              onTap: () {
                widget.onColorChanged(c['nameEn'] as String);
                setState(() => _showCustomInput = false);
              },
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      color: sel ? const Color(0xFF1A1A1A) : const Color(0xFF555555),
                      height: 1.2,
                    ),
                    textAlign: TextAlign.center,
                    maxLines: 2,
                  ...
                  if (widget.isBottomCategory)
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 7.5,
                        fontWeight: FontWeight.w700,
                        color: isFree ? const Color(0xFF2E7D32) : const Color(0xFFD84315),
                      ),
                    ),
                ],
              ),
        ...
        // ── 직접입력 패널 ──
        if (_showCustomInput && widget.isBottomCategory) ...[
          const SizedBox(height: 12),
          Container(
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    const Icon(Icons.palette_outlined, size: 14, color: Color(0xFF666666)),
                    const SizedBox(width: 6),
                    Text(loc.colorCodeInput, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF333333))),
                    const SizedBox(width: 6),
                    Text(loc.productHexCodeLabel, style: const TextStyle(fontSize: 11, color: Color(0xFF888888))),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    // 색상 미리보기 (골지 텍스처)
                    GestureDetector(
                      onTap: _previewColor != null ? _applyCustomColor : null,
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: SizedBox(
                          width: 44, height: 44,
                          child: Stack(
                            fit: StackFit.expand,
                            children: [
                              ColoredBox(color: _previewColor ?? const Color(0xFFEEEEEE)),
                              if (_previewColor != null)
                                CustomPaint(painter: RibTexturePainter(baseColor: _previewColor!)),
                              if (_previewColor == null)
                                const Center(child: Icon(Icons.colorize_rounded, color: Color(0xFFBBBBBB), size: 20)),
                            ],
                          ),
                        ),
                      ),
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: '#1A237E',
                          hintStyle: const TextStyle(color: Color(0xFFBBBBBB), fontWeight: FontWeight.normal),
                          prefixText: '#',
                          prefixStyle: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF666666)),
                          counterText: '',
                          errorText: _hexError,
                    ...
                    const SizedBox(width: 8),
                    ElevatedButton(
                      onPressed: _previewColor != null ? _applyCustomColor : null,
                      style: ElevatedButton.styleFrom(
                        backgroundColor: _previewColor ?? const Color(0xFFCCCCCC),
                        foregroundColor: Colors.white,
                        elevation: 0,
                        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 14),
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                      ),
                      child: Text(loc.applyLabel, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
                    ),
                  ],
                ),
                const SizedBox(height: 8),
                ...
                const Row(
                  children: [
                    Icon(Icons.info_outline_rounded, size: 12, color: Color(0xFFD84315)),
                    SizedBox(width: 4),
                    Expanded(
                      child: Text(
                        '직접 입력 색상은 +₩20,000 추가비용이 발생합니다. 색상 팔레트 참고: colorpicker.me',
                        style: TextStyle(fontSize: 10, color: Color(0xFFD84315)),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 6),
                // 자주 쓰는 색상 빠른 선택
                Text(loc.frequentColors, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF888888))),
                const SizedBox(height: 6),
                Wrap(
                  ...
                  children: const [
                    ...
                    onTap: () {
                      _hexController.text = item['hex']!;
                      _parseHex(item['hex']!);
                    },
                    child: RibColorSwatch(
                      color: Color(int.parse('FF${item["hex"]}', radix: 16)),
                      size: 28,
                      isLight: Color(int.parse('FF${item["hex"]}', radix: 16)).computeLuminance() > 0.5,
                    ),
                  )).toList(),
                ),
              ],
        ...
        if (widget.selectedColor != null) ...[
          ...
          Builder(builder: (ctx) {
            ...
            final isCustom = col.startsWith('#');
            Color displayColor;
            String displayName;
            bool isFree = false;
            if (isCustom) {
              try {
                displayColor = Color(int.parse('FF${col.substring(1)}', radix: 16));
              } catch (_) {
                displayColor = Colors.grey;
              }
              displayName = '직접 입력: $col';
              isFree = false;
            } else {
              final found = widget.isBottomCategory
                  ? _palette.firstWhere((c) => c['nameEn'] == col, orElse: () => _defaultColors.firstWhere((c) => c['nameEn'] == col, orElse: () => <String, dynamic>{}))
                  : _defaultColors.firstWhere((c) => c['nameEn'] == col, orElse: () => <String, dynamic>{});
              if (found.isEmpty) return const SizedBox.shrink();
              displayColor = Color(found['hex'] as int);
              displayName = found['name'] as String;
              isFree = found['free'] as bool;
            }
            return Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text('${loc.selectedLabel} $displayName', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF1A1A1A))),
                  ),
                  if (widget.isBottomCategory)
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                      decoration: BoxDecoration(
                        color: isFree ? const Color(0xFF4CAF50).withValues(alpha: 0.15) : const Color(0xFFFF5722).withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(20),
                      ),
                      child: Text(
                        isFree ? '무료' : '+₩20,000',
