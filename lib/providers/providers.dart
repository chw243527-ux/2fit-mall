...
class CartProvider extends ChangeNotifier {
  ...
  Future<void> _restoreCart() async {
    try {
      ...
      for (final encoded in raw) {
        ...
        _items.add(CartItem(
          id: data['id']?.toString() ??
              '${productData['id']}_${DateTime.now().microsecondsSinceEpoch}',
  ...
  Future<void> _persistCart() async {
    _persistQueue = _persistQueue.then((_) async {
      ...
      final snapshot = _items
          ...
          .toList();
      await prefs.setStringList('twofit_cart_v1', snapshot);
    });
    await _persistQueue;
  }
  ...
  void _syncProducts(List<ProductModel> products) {
    ...
    for (final item in _items) {
      ...
      item.product = latest;
      final optionStock =
          latest.stockData[item.selectedSize]?[item.selectedColor];
  ...
  void addItem(ProductModel product, String size, String color,
    ...
    _items.add(CartItem(
      id: '${product.id}_${size}_${color}_${DateTime.now().microsecondsSinceEpoch}',
      product: product,
      selectedSize: size,
      selectedColor: color,
      quantity: quantity,
  ...
  void updateQuantity(String itemId, int quantity) {
    ...
    final optionStock =
        item.product.stockData[item.selectedSize]?[item.selectedColor];
    // 상품 스트림으로 최신 상품이 반영된 경우에도 장바구니 수량이
    // 현재 사이즈×색상 재고를 초과하지 않도록 제한합니다.
    final cappedQuantity =
        optionStock == null ? quantity : quantity.clamp(0, optionStock).toInt();
    if (cappedQuantity <= 0) {
      removeItem(itemId);
      return;
    }
