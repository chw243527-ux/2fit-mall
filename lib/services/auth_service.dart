...
class AuthService {
  ...
  // ────────────────────────────────────────────
  // 로컬 테스트 계정 (Firebase 연결 불가 시 폴백)
  // ────────────────────────────────────────────
  static const _localAccounts = [
    {
      'email': 'admin@2fit.co.kr',
      'password': 'admin2fit!',
      'name': '관리자',
      'isAdmin': true,
    },
    {
      'email': 'manager@2fit.co.kr',
      'password': 'manager2fit!',
      'name': '매니저',
      'isAdmin': true,
    },
    {
  ...
  static AuthResult? _tryLocalLogin(String emailKey, String password) {
    for (final acc in _localAccounts) {
      if (acc['email'] == emailKey && acc['password'] == password) {
        final isAdmin = acc['isAdmin'] as bool;
        final user = UserModel(
          id: 'local_${emailKey.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_')}',
