...
extension PointActionTypeExt on PointActionType {
  String get label {
    switch (this) {
      case PointActionType.earn:
        return '적립';
      case PointActionType.use:
        return '사용';
      case PointActionType.expire:
        return '소멸';
      case PointActionType.admin:
        return '관리자 조정';
    }
  }
  ...
  static PointActionType fromKey(String k) {
    switch (k) {
      case 'use':
        return PointActionType.use;
      case 'expire':
        return PointActionType.expire;
      case 'admin':
        return PointActionType.admin;
      default:
        return PointActionType.earn;
    }
  }
}
...
class PointService {
  ...
  static Stream<List<PointHistory>> watchHistory(String userId) {
    return _db
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => PointHistory.fromDoc(d.id, d.data())).toList())
        .handleError((e) {
      if (kDebugMode) debugPrint('client_operation_failed');
      return <PointHistory>[];
    });
  ...
  static Future<bool> refundPoints({
    ...
    try {
      final userRef = _db.collection('users').doc(userId);
      final refundRef =
          userRef.collection('point_history').doc('refund_$orderId');
      await _db.runTransaction((txn) async {
      ...
      for (final doc in historySnap.docs) {
        ...
        if (source.amount <= 0 ||
            (source.action != PointActionType.earn &&
                source.action != PointActionType.admin)) {
          continue;
        }
        final expireRef = _db
            .collection('users')
        ...
        await _db.runTransaction((txn) async {
          ...
          txn.set(
              ...
              PointHistory(
                ...
                amount: -amount,
                desc:
                    '${source.expiresAt!.year}.${source.expiresAt!.month}.${source.expiresAt!.day} 적립 포인트 기간 만료 소멸',
