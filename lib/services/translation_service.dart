...
class TranslationService {
  ...
  static Future<Map<String, String>> translateWithCache(
      String koreanText) async {
  ...
  static Future<String?> translateSingle(
      String koreanText, String targetLangCode) async {
    final key = koreanText.trim();
    if (key.isEmpty) return null;
    // 캐시에 해당 언어 번역이 있으면 즉시 반환
    if (_cache[key]?[targetLangCode] != null)
      return _cache[key]![targetLangCode];
    final langPair = _langCodes[targetLangCode];
    if (langPair == null) return null;
    final result = await _translate(key, langPair);
    if (result != null && result.isNotEmpty) {
      _cache.putIfAbsent(key, () => <String, String>{})[targetLangCode] =
          result;
    }
    return result;
  }
  ...
  static Future<Map<String, String>> translateLongText(
      String koreanText) async {
    if (koreanText.trim().isEmpty) return {};
